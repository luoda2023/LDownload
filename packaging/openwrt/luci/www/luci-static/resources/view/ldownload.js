...
/*
 * luci-app-ldownload — 服务开关/监听地址/数据目录配置 + Web 界面跳转。
 * 完整下载管理界面由 ldownload-server 自带的 React SPA 提供，
...
return view.extend({
	render: function() {
		...
		m = new form.Map('ldownload', _('LDownload'),
		...
		s = m.section(form.NamedSection, 'main', 'ldownload', _('Service settings'));

		o = s.option(form.Flag, 'enabled', _('Enabled'),
			_('Start the LDownload server on boot.'));
		o.rmempty = false;
		o.default = '1';
		...
		o.placeholder = '/etc/ldownload';
		o.rmempty = true;

		o = s.option(form.Button, '_webui', _('Web interface'));
		o.inputtitle = _('Open LDownload');
		o.inputstyle = 'apply';
		o.onclick = function() {
			var port = bindToPort(uci.get('ldownload', 'main', 'bind'));
