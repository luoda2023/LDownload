...
#
# 前置：imagemagick（convert）用于从 assets/logo/ldownload_logo.png 生成套件图标。
set -eu
...
# ── package.tgz 载荷：bin/ldownload-server（Web UI 已编译期内嵌，无 web/ 目录）──
mkdir -p "$payload/bin"
cp "$BIN" "$payload/bin/ldownload-server"
chmod 755 "$payload/bin/ldownload-server"

# ── ui/：DSM 桌面应用入口。官方要求该目录在 package.tgz 内（安装后位于
#    /var/packages/LDownload/target/ui），DSM 依 INFO 的 dsmuidir 将其软链到
#    /usr/syno/synoman/webman/3rdparty/LDownload，主菜单/导航窗格才会出图标。
...
{
	echo 'package="LDownload"'
	echo "version=\"$SPK_VERSION\""
	echo 'displayname="LDownload Server"'
	...
	echo 'maintainer="luoda2023"'
	echo 'maintainer_url="https://dicad.cn"'
	echo 'support_url="https://github.com/luoda2023/LDownload/issues"'
	echo "arch=\"$ARCH\""
	echo 'thirdparty="yes"'
	echo 'startable="yes"'
	echo 'adminport="17800"'
	echo 'dsmuidir="ui"'
	echo 'dsmappname="com.ldownload.server"'
