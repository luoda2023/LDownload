...
// 注意不得等于 L"LDownload"：main.cpp 单实例逻辑用
// FindWindow(class, L"LDownload") 定位主窗口，两者共享窗口类。
constexpr const wchar_t kPopupTitle[] = L"LDownload Quick Download";
...
PopupWindowHost::PopupWindowHost(flutter::BinaryMessenger* host_messenger) {
  host_channel_ =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          host_messenger, "ldownload/popup_host",
...
bool PopupWindowHost::OnCreate() {
  ...
  child_channel_ =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          flutter_controller_->engine()->messenger(), "ldownload/popup_child",
          &flutter::StandardMethodCodec::GetInstance());
