...
//   缺失或空串的键按键级回退到英文。
// - 解析顺序：localStorage `ldownload.locale`（用户显式选择）→ 服务器默认语言
//   （无鉴权 `/ping` 的 language，实时求值：设置页保存的 `web_language` 优先，
//   未保存时回退部署环境 LDOWNLOAD_LANG；登录页同样生效）→ 浏览器语言 → en。
...
const LOCALE_KEY = 'ldownload.locale'
...
const BACKEND_KEYS: Record<string, I18nKey> = {
  ...
  'missing X-LDownload-Client header': 'backend.missingClientHeader',
