 ...
 *     <-> fetch() HTTP/JSON（X-LDownload-Token 鉴权）
 *   ldown_server（headless 下载服务，与桌面 App 并列的第二条投递通道）
 *
 * 设计决策：
...
/** remote-server 所需的最小配置（对应 LDownloadSettings 的 remoteUrl/remoteToken 子集） */
export interface RemoteServerConfig {
  /** ldown_server 地址，如 http://192.168.1.10:17800（不带尾部斜杠） */
  remoteUrl: string;
...
function buildHeaders(cfg: RemoteServerConfig): HeadersInit {
  return {
    ...
    "X-LDownload-Client": "extension",
    "X-LDownload-Token": cfg.remoteToken,
  };
}
 ...
 * ldown_server 的管理 API 恒开且强制 token，/api/v1/info 是最轻量的
