...
export interface LDownloadSettings {
  /** 是否启用下载拦截 */
  enabled: boolean;
  /** 拦截模式 */
   ...
   * 处理程序，改由 LDownload 下载）。关闭后点击磁力链接走系统默认
   * 处理程序（如 qBittorrent）；资源面板仍会展示探测到的磁力资源，
   * 供手动选择用 LDownload 下载。默认开启。
   */
  ...
  /** 远程 ldown_server 地址，如 http://192.168.1.10:17800（保存时会去除尾部斜杠） */
  remoteUrl: string;
  /** 远程 ldown_server 鉴权 token */
  remoteToken: string;
  /**
  ...
  /**
   * 启用 ldownload:// 自定义协议下载（Android 与桌面均生效）。
   * 开启后拦截下载时不再投递 NMH/远端，而是导航到
   * ldownload://download?url=... 协议 URL，由系统唤起已注册该协议的
   * LDownload 客户端（Android: VIEW intent；桌面: Windows 注册表 /
   ...
   * 受限环境开启。默认关闭。
   */
  enableLdownloadProtocol: boolean;
}
...
const DEFAULT_SETTINGS: LDownloadSettings = {
  ...
  // 自定义协议
  enableLdownloadProtocol: false,
};

/**
 * 加载设置
 */
export async function loadSettings(): Promise<LDownloadSettings> {
...
export async function saveSettings(
  settings: Partial<LDownloadSettings>,
): Promise<void> {
