 ...
 * 通过 Native Messaging 协议与 LDownload 桌面应用通信
 *
 * 通信链路：
 *   Browser Extension
 *     <-> browser.runtime.connectNative() (stdin/stdout, 4字节LE长度前缀+JSON)
 *   ldown_nmh.exe (中继进程)
 *     <-> Named Pipe \\.\pipe\ldownload (4字节LE长度前缀+JSON)
 *   LDownload App
 *
 * 设计决策：
...
const NMH_NAME = "com.ldownload.nmh";
...
export interface DownloadRequest {
   ...
   * 在 form-POST 触发的下载场景下必传，否则 LDownload 会用 GET 重发拿到错误内容。
   */
  method?: string;
...
export interface ApiResponse {
   ...
   * "local"=桌面 App（NMH），"remote"=远程 ldown_server。
...
function getPort(): chrome.runtime.Port | null {
  ...
  try {
    ...
    console.error("[LDownload NMH] connectNative() threw:", e);
    return null;
  }
  ...
  _port.onDisconnect.addListener((p) => {
    ...
    if (err?.message) {
      console.error("[LDownload NMH] port disconnected, reason:", err.message);
    } else {
      console.warn("[LDownload NMH] port disconnected (no error reason)");
    }
    _port = null;
...
async function sendWithRetry(
  ...
  if (maybeDelivered && action !== "ping") {
    ...
    if (pingResult.success) {
      console.log(
        `[LDownload NMH] App alive after "${result.message}" — message likely delivered, skipping retry`,
      );
      return {
        success: true,
...
export async function nmhCheckLDownloadAvailable(): Promise<boolean> {
 ...
 * 带一次重连重试的可用性探测。
 *
 * 与 nmhCheckLDownloadAvailable 的区别：首次 ping 失败（超时/端口断开/app_not_running）时，
 ...
 */
export async function nmhCheckLDownloadAvailableWithRetry(): Promise<boolean> {
