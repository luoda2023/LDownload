...
import type { LDownloadSettings, RemoteMode } from "./settings";
...
let _settingsCache: LDownloadSettings | null = null;

async function getRoutingSettings(): Promise<LDownloadSettings> {
...
function toRoutingConfig(settings: LDownloadSettings): RoutingConfig {
  return {
...
export async function checkLDownloadAvailable(): Promise<boolean> {
  const cfg = toRoutingConfig(await getRoutingSettings());
  const mode = effectiveMode(cfg);

  if (mode === "off") {
    return nmh.nmhCheckLDownloadAvailable();
  }
  if (mode === "always") {
  ...
  const [nmhUp, remoteUp] = await Promise.all([
    nmh.nmhCheckLDownloadAvailable().catch(() => false),
 ...
 */
export async function checkLDownloadAvailableWithRetry(): Promise<boolean> {
  const cfg = toRoutingConfig(await getRoutingSettings());
  const mode = effectiveMode(cfg);

  if (mode === "off") {
    return nmh.nmhCheckLDownloadAvailableWithRetry();
  }
  if (mode === "always") {
    const result = await remotePing(cfg.remote);
    return result.success === true;
  ...
  const [nmhUp, remoteUp] = await Promise.all([
    nmh.nmhCheckLDownloadAvailableWithRetry().catch(() => false),
    remotePing(cfg.remote)
      .then((r) => r.success === true)
      .catch(() => false),
  ]);
