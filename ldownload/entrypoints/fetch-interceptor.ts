...
// 本文件新增的响应体流式部分读取为 LDownload 自研，非 cat-catch 代码移植。
...
export default defineUnlistedScript(() => {
  // 防止重复注入
  if ((window as any).__ldownload_interceptor__) return;
  (window as any).__ldownload_interceptor__ = true;

  const LDOWNLOAD_EVENT = "ldownload-resource-detected";
  // 拦到标准 DASH JSON manifest 时派发：权威清晰度 + 视频/音频轨 URL 列表，
  // 供 Isolated World 转发给 background 存入 tab 级 manifest store。
  const LDOWNLOAD_DASH_EVENT = "ldownload-dash-manifest";
  ...
  function notify(
    ...
    document.dispatchEvent(
      new CustomEvent(LDOWNLOAD_EVENT, {
  ...
  function tryDashManifest(obj: unknown, url: string): void {
    try {
      ...
      if (manifest) {
        document.dispatchEvent(
          new CustomEvent(LDOWNLOAD_DASH_EVENT, {
  ...
  XMLHttpRequest.prototype.open = function (
    ...
    try {
      ...
      (this as any).__ldownload_url = urlStr;
  ...
  XMLHttpRequest.prototype.send = function (...args: any[]) {
    this.addEventListener("load", function () {
      try {
        const url = (this as any).__ldownload_url as string | undefined;
        if (!url) return;
  ...
  // 通过拦截 AJAX 响应，在浏览器发起 CDN GET 之前将 URL 发给 LDownload，
  // 并由 background 通过 declarativeNetRequest 阻断浏览器的 CDN 请求，
  // 确保 LDownload 是第一个（也是唯一的）请求方。
  //
  // 目前支持的规则：
  ...
  function notifyPreemptDownload(json: any, apiUrl: string): void {
    ...
    if (
      ...
      document.dispatchEvent(
        new CustomEvent("ldownload-preempt-download", {
          detail: {
            url: cdnUrl,
  ...
  console.log("[LDownload] Fetch/XHR/Blob/MSE interceptor injected");
});
