/**
 * LDownload Background Service Worker
 *
 * 职责：
 * 1. 拦截浏览器下载事件，转发给 LDownload 桌面应用
 * 2. 注册右键菜单（发送链接到 LDownload）
...
import {
  ...
  checkLDownloadAvailable,
  checkLDownloadAvailableWithRetry,
  warmupNativeHost,
...
export default defineBackground(() => {
  console.log("[LDownload] Background service worker started");

  // ===== P3: settings 内存缓存 =====
  // 避免每次拦截都 await chrome.storage.sync.get（热路径去异步化）
  // storage.onChanged 保证跨标签页/窗口的实时同步
  let _settingsCache: import("@/utils/settings").LDownloadSettings | null = null;
  ...
  let _settingsInflight: Promise<
    import("@/utils/settings").LDownloadSettings
  > | null = null;
  ...
  getCachedSettings()
    .then((s) => {
      ...
      syncDownloadShelfState(s.enabled);
      console.log("[LDownload] Settings cache warmed up");
    })
    .catch((e) => {
      console.warn("[LDownload] Settings cache warmup failed (non-fatal):", e);
    });
  ...
  initI18n()
    .then(() => {
      console.log("[LDownload] i18n initialized");
      // i18n 就绪后创建右键菜单（菜单标题需要翻译）
      createContextMenus();
    })
    .catch((e) => {
      console.warn("[LDownload] i18n init failed (non-fatal):", e);
    });
  ...
  // ===== 右键菜单：即使关闭自动拦截也可以手动发送链接到 LDownload 下载 =====
  ...
  function createContextMenus() {
    if (!browser.contextMenus) {
      console.warn("[LDownload] contextMenus API not available");
      return;
    }
    browser.contextMenus
      ...
      .then(() => {
        browser.contextMenus.create({
          id: "ldownload-send-link",
          title: t("contextMenu.sendToLDownload"),
          contexts: ["link"],
        });
        browser.contextMenus.create({
          id: "ldownload-send-image",
          title: t("contextMenu.sendImageToLDownload"),
          contexts: ["image"],
        });
        browser.contextMenus.create({
          id: "ldownload-send-video",
          title: t("contextMenu.sendVideoToLDownload"),
          contexts: ["video", "audio"],
        });
        browser.contextMenus.create({
          id: "ldownload-send-page",
          title: t("contextMenu.sendPageToLDownload"),
          contexts: ["page"],
        });
        console.log("[LDownload] Context menus created");
      })
      .catch((e: unknown) => {
        console.warn("[LDownload] Failed to create context menus:", e);
      });
  }
    ...
    browser.contextMenus.onClicked.addListener(
      async (info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab) => {
        ...
        switch (info.menuItemId) {
          case "ldownload-send-link":
            downloadUrl = info.linkUrl;
            break;
          case "ldownload-send-image":
          case "ldownload-send-video":
            downloadUrl = info.srcUrl;
            break;
          case "ldownload-send-page":
            downloadUrl = info.pageUrl;
            break;
          default:
            return; // 非 LDownload 菜单项，忽略
        }
        ...
        console.log(
          "[LDownload] Context menu download:",
          info.menuItemId,
          downloadUrl,
        );

        const sendOk = await sendToLDownload(downloadUrl, referrer);
        // 成功提示已收敛到 sendToLDownload 内部统一发送，此处只处理失败回退。
        if (!sendOk) {
  ...
  browser.commands.onCommand.addListener(async (command) => {
    ...
    console.log("[LDownload] Intercept toggled via shortcut:", newEnabled);
  });
  ...
  // 触发下载的站点，LDownload 拿到 URL 后用 GET 重发会拿到 HTML 页面而非
  ...
  // method/body 一起塞进发送给 LDownload App 的 payload。Rust 端用此
  // 一比一重建浏览器请求事务。
  //
  ...
  try {
    ...
    console.log(
      "[LDownload] webRequest.onBeforeRequest listener registered (with requestBody)",
    );
  } catch (e) {
    console.warn(
      "[LDownload] Failed to register onBeforeRequest listener:",
      e,
    );
  }
  ...
  try {
    ...
    console.log(
      "[LDownload] webRequest.onSendHeaders listener registered (with extraHeaders)",
    );
  } catch {
    try {
      browser.webRequest.onSendHeaders.addListener(
      ...
      console.log(
        "[LDownload] webRequest.onSendHeaders listener registered (without extraHeaders)",
      );
    } catch (e) {
      console.warn(
        "[LDownload] Failed to register webRequest.onSendHeaders listener:",
        e,
      );
    }
  }
   ...
   * 其余（Accept、Sec-Fetch-*、Accept-Language 等）全部原样传递给 LDownload。
   *
  ...
  try {
    browser.webRequest.onHeadersReceived.addListener(
      (details) => {
        ...
        console.log(
          "[LDownload] Detected download-triggering response (onHeadersReceived):",
          info,
        );
    ...
    console.log("[LDownload] webRequest.onHeadersReceived listener registered");
  } catch (e) {
    console.warn(
      "[LDownload] Failed to register webRequest.onHeadersReceived listener:",
      e,
    );
  }
  ...
  try {
    browser.webRequest.onHeadersReceived.addListener(
      (details) => {
        ...
        // 仍能携带正确的认证头发送给 LDownload——这是 IDM 能成功而普通插件失败的关键。
    ...
    console.log(
      "[LDownload] Resource sniffer (onHeadersReceived for media) registered",
    );
  } catch (e) {
    console.warn("[LDownload] Failed to register resource sniffer:", e);
  }
  ...
  // onDeterminingFilename 检查此表，避免重复发送给 LDownload。
  ...
  const APP_DOWN_STATE_KEY = "ldownloadAppDownUntil";
  let _appDownUntil = 0;
  ...
  function probeAppReachable(): Promise<boolean> {
    if (!_appProbe) {
      _appProbe = checkLDownloadAvailableWithRetry()
        .catch(() => false)
        .finally(() => {
          _appProbe = null;
        });
  ...
  // 同步做拦截决策，异步 fire-and-forget 发送到 LDownload（不阻塞响应管线）。
  //
  ...
  if (!hasDeterminingFilename) {
    const onDownloadHeadersBlocking = (
      ...
      // 命中：同步取消该请求（浏览器不创建下载项），异步发送到 LDownload
      console.log("[LDownload] Firefox blocking intercept (cancel at source):", {
        url: details.url,
        contentType,
        contentLength,
        dispositionFilename,
      ...
      sendToLDownload(
        ...
        .then((ok) => {
          if (!ok) {
            console.warn(
              "[LDownload] Firefox blocking: send failed, falling back to browser download:",
              details.url,
            );
        ...
        .catch((e) => {
          console.error(
            "[LDownload] Firefox blocking: sendToLDownload threw:",
            e,
          );
    ...
    try {
      ...
      console.log(
        "[LDownload] Firefox blocking download interceptor registered",
      );
    } catch (e) {
      console.warn(
        "[LDownload] Failed to register Firefox blocking interceptor:",
        e,
      );
    }
  }
  ...
  browser.downloads.onCreated.addListener((downloadItem) => {
    ...
    console.log("[LDownload] onCreated:", {
      id: downloadId,
      url,
    ...
    startFallbackInterception(downloadId, downloadItem).catch((e) => {
      console.error(
        "[LDownload] Unexpected error in startFallbackInterception:",
        e,
      );
    });
  ...
  async function startFallbackInterception(
    ...
    console.log("[LDownload] startFallbackInterception:", {
      id: downloadId,
      url,
  ...
  async function handleResponseCacheHit(
    ...
    if (handledDownloads.has(downloadId)) return;
    console.log("[LDownload] Fallback path A (response cache):", {
      id: downloadId,
      url,
    ...
    console.log("[LDownload] Path A shouldIntercept:", intercept, itemInfo);
    if (!intercept) return;
    if (handledDownloads.has(downloadId)) return;
    ...
    try {
      await executeFallbackIntercept(
        ...
        itemInfo,
        // 重定向场景：传入原始 URL，让 sendToLDownload 可回退查找 headers 缓存
  ...
  async function handleDownloadMetaHit(
    ...
    if (handledDownloads.has(downloadId)) return;
    console.log("[LDownload] Fallback path B (download meta):", {
      id: downloadId,
    ...
    if (
      ...
      console.log(
        "[LDownload] Path B: download already complete/interrupted, skip",
      );
      return;
    }
    ...
    const intercept = shouldIntercept(itemInfo, settings);
    console.log("[LDownload] Path B shouldIntercept:", intercept, itemInfo);
    if (!intercept) return;
    if (handledDownloads.has(downloadId)) return;
  ...
  /**
   * 执行兜底拦截：cancel + erase + 发送到 LDownload
   *
   * 策略：先取消浏览器下载，再发送到 LDownload。
   * 原因：如果先发送再取消，异步等待期间浏览器下载持续进行，
   * 小文件可能在 cancel 前已完成，导致"双下载"（用户同时看到
   * LDownload 下载和浏览器已完成的下载）。Firefox 尤为严重，
  ...
  async function executeFallbackIntercept(
    ...
    try {
      ...
      console.error(
        "[LDownload] executeFallbackIntercept: sendToLDownload threw:",
        e,
      );
    }
    ...
    if (!sendOk) {
      ...
      console.warn(
        "[LDownload] executeFallbackIntercept: send failed, checking app status before fallback:",
        url,
      );
      handledDownloads.delete(downloadId);
    ...
    browser.downloads.onDeterminingFilename.addListener(
      (downloadItem, suggest) => {
        ...
        if (handledDownloads.get(downloadItem.id) === "fallback") {
          console.log(
            "[LDownload] onDeterminingFilename: already handled by fallback, cancelling:",
            downloadItem.id,
          );
        ...
        if (preemptEntry && preemptEntry.expiry > Date.now()) {
          if (downloadUrl === url) {
            ...
            console.log(
              "[LDownload] onDeterminingFilename: preempted URL, no redirect detected, letting browser handle:",
              url,
            );
          ...
          console.log(
            "[LDownload] onDeterminingFilename: preempted URL redirected, intercepting finalUrl:",
            downloadUrl,
          );
        }
        ...
        if (_syncSettings !== null) {
          ...
          if (shouldIntercept(_syncItemInfo, _syncSettings)) {
            ...
            suggest();
            console.log("[LDownload] Intercepting download (sync-path):", {
              url,
              downloadUrl,
              mime: _syncMime,
              filename: _syncFilename,
            ...
            (async () => {
              try {
                try {
                  ...
                  console.debug(
                    "[LDownload] sync-path: cancel after suggest (expected)",
                  );
                }
                try {
                  ...
                  console.debug(
                    "[LDownload] sync-path: erase after cancel (expected)",
                  );
                }
                ...
                const sendOk = await sendToLDownload(
                  downloadUrl,
                  _syncReferrer,
                  _syncClean,
                  _syncFileSize,
                ...
                console.error("[LDownload] sync-path: sendToLDownload error:", e);
        ...
        if (_syncSettings === null) {
          ...
          suggest();
          console.log(
            "[LDownload] Cold-start pre-emptive intercept (settings cache not warmed):",
            { url, downloadUrl },
          );
          (async () => {
            try {
              ...
              try {
                ...
                console.debug("[LDownload] cold-start: cancel (expected)");
              }
              try {
                await browser.downloads.erase({ id: downloadItem.id });
              } catch {
                console.debug("[LDownload] cold-start: erase (expected)");
              }
              ...
              console.error(
                "[LDownload] Cold-start pre-emptive intercept error:",
                e,
              );
              handledDownloads.delete(downloadItem.id);
              // 异常情况：先 ping 确认 App 是否在线再决定是否回退
        ...
        (async () => {
          ...
          // 防止 sendToLDownload 失败时 catch 块误删 handledDownloads 标记导致重复发送。
          ...
          const callSuggestCancel = async () => {
            ...
            try {
              ...
              console.debug(
                "[LDownload] async-path: cancel after suggest (expected)",
              );
            }
            try {
              await browser.downloads.erase({ id: downloadItem.id });
              ...
              console.debug(
                "[LDownload] async-path: erase after cancel (expected)",
              );
            }
          };
          ...
          try {
            ...
            console.log(
              "[LDownload] Intercepting download (onDeterminingFilename):",
              {
                url,
                downloadUrl,
                mime,
            ...
            console.error(
              "[LDownload] Error in onDeterminingFilename handler:",
              e,
            );
    ...
    _queueChain = _queueChain.then(async () => {
      try {
        ...
        if (!isDup) {
          ...
          console.log("[LDownload] Enqueued pending request:", request.url);
        }
      } catch (e) {
        console.warn("[LDownload] Failed to enqueue pending request:", e);
      }
    });
    return _queueChain;
  }
    ...
    _queueChain = _queueChain.then(async () => {
      ...
      for (const item of queue) {
        if (item.retries >= MAX_PENDING_RETRIES) {
          console.warn(
            "[LDownload] Pending request exceeded max retries, dropping:",
            item.request.url,
          );
          continue; // 丢弃
        }
      ...
      for (let i = 0; i < toRetry.length; i++) {
        ...
        if (result.status === "fulfilled" && result.value.success) {
          console.log("[LDownload] Pending request flushed:", item.request.url);
  ...
  // 活跃感知来源：
  //   a) 本地下载请求发送成功后（sendToLDownload / batchDownload），乐观地
  //      认为"可能有活跃任务"，启动 alarm；
  ...
  const TASK_POLL_ALARM_NAME = "ldownload-task-poll";
  ...
  async function ensureTaskPollAlarm(): Promise<void> {
    try {
      ...
      console.warn("[LDownload] ensureTaskPollAlarm failed:", e);
    }
  }
  ...
  // storage.local。
  const TASK_SNAPSHOT_SESSION_KEY = "ldownloadTaskSnapshot";
  const TASK_SNAPSHOT_LOCAL_KEY = "ldownloadTaskSnapshotFallback";
  ...
  const TASK_NOTIFICATION_PREFIX = "ldownload-task-";
  ...
  function notifyTaskCompleted(task: TaskBrief): void {
    ...
    try {
      ...
      console.warn("[LDownload] notifyTaskCompleted: failed to create:", e);
    }
  }
  ...
  browser.alarms.onAlarm.addListener((alarm) => {
    ...
    nmhListTasks()
      ...
      .catch((e) => {
        console.warn("[LDownload] task poll alarm handler failed:", e);
      });
  });
  ...
  // ldownload:// 协议（Windows 注册表 / Linux x-scheme-handler / macOS
  ...
  /**
   * 打开 ldownload://download?url=...&filename=... 协议 URL，触发系统唤起
  ...
  async function openProtocolUrl(
    ...
    const protocolUrl = `ldownload://download?${params.toString()}`;
    console.log("[LDownload] protocol mode — opening:", protocolUrl);
    try {
      ...
      console.warn("[LDownload] failed to open ldownload:// URL:", e);
      return false;
    }
  }

  // ===== 核心：发送下载请求到 LDownload App =====
  async function sendToLDownload(
    url: string,
    referrer?: string,
    filename?: string,
    fileSize?: number,
    ...
    // === ldownload:// 自定义协议模式 ===
    // 开启后跳过 NMH/远端投递，经协议 URL 唤起 LDownload 客户端
    ...
    // 限制：不经过 NMH，Cookie/Headers/method/body 无法携带，适用于公开文件。
    {
      ...
      if (protocolSettings.enableLdownloadProtocol) {
        if (audioUrl) {
          // 音视频分轨对（video+audio mux）无法经协议 URL 表达 —— 丢弃
          // audioUrl 会下成无声视频。返回失败让调用方回退浏览器下载。
          console.warn(
            "[LDownload] protocol mode cannot carry audioUrl; falling back to browser download",
          );
          ...
          if (ok && (await shouldNotifyChannel("local"))) {
            ...
            notify(
              ...
              t("notify.sentToLDownload", { name: shownName }),
            );
          }
          return ok;
        }
    ...
    if (authFromCache.cookies || authFromCache.headers) {
      console.log(
        "[LDownload] Cookies from webRequest cache:",
        cookieString.length,
        "chars",
      );
      console.log(
        "[LDownload] Extra headers from webRequest cache:",
    ...
    if (!cookieString) {
      try {
        ...
        cookieString = cookiesResult
          ...
          .join("; ");
        console.log(
          "[LDownload] Cookies from cookies API:",
          cookiesResult.length,
          "cookies",
        );
      } catch (e) {
        console.warn(
          "[LDownload] Failed/timeout to extract cookies via API:",
          e,
        );
      }
    }
    ...
    if (!cookieString && storedCookies) {
      ...
      console.log(
        "[LDownload] Cookies from stored resource:",
        cookieString.length,
        "chars",
      );
    }
    if (
      ...
      console.log(
        "[LDownload] Extra headers from stored resource:",
        Object.keys(extraHeaders).length,
      );
    }
    ...
    if (reqRecord.method || reqRecord.body) {
      console.log(
        "[LDownload] Captured original request transaction: method=",
    ...
    console.log("[LDownload] Sending to LDownload app:", request);
    ...
    if (response.success) {
      ...
      await incrementStat("failed");
      console.warn(
        "[LDownload] sendToLDownload failed:",
        response.message,
        "url:",
        url,
      );
  ...
  /**
   * 回退到浏览器下载：当发送到 LDownload 失败时，重新发起浏览器下载。
  ...
  async function fallbackToBrowserDownload(
    ...
    try {
      ...
      console.log("[LDownload] Fallback: re-initiated browser download:", url);
    } catch (e) {
      console.error(
        "[LDownload] Fallback: failed to re-initiate browser download:",
        e,
      );
    }
    if (!silent) {
  ...
  /**
   * sendToLDownload 失败后的智能回退：先 ping 确认 App 状态再决定是否回退。
   *
  ...
  async function fallbackAfterSendFailure(
    ...
    try {
      ...
      if (appAlive) {
        ...
        markAppUp();
        console.log(
          "[LDownload] App is alive after send failure — skipping browser fallback (message likely delivered):",
          url,
        );
        return;
      }
  ...
  async function handleMessage(
    ...
    switch (message.action) {
      // --- Popup 消息 ---
      case "getStatus": {
        const available = await checkLDownloadAvailable();
      ...
      case "checkConnection": {
        const isAvailable = await checkLDownloadAvailable();
      ...
      // 远程通道（ldown_server）：单次 POST /download/batch 发送全部条目。
      case "batchDownload": {
        ...
        // Bug 9 修复：cookies API 加 500ms 超时，与 sendToLDownload 保持一致
      ...
      // 才能获取真正的下载 URL。因此不再发送给 LDownload，仅记录 URL 做去重。
      ...
      case "preemptDownload": {
        ...
        console.log(
          "[LDownload] preemptDownload: recorded URL (not sent to LDownload, browser will load transit page):",
          preemptUrl,
        );
  ...
  async function cancelAndErase(downloadId: number): Promise<void> {
    try {
      ...
      console.warn("[LDownload] cancelAndErase: cancel failed:", e);
    }
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        ...
        console.warn("[LDownload] cancelAndErase: erase failed:", e);
      }
      try {
    ...
    console.warn(
      "[LDownload] cancelAndErase: record still present after retries:",
      downloadId,
    );
  }
  ...
  function notify(title: string, message: string) {
    ...
    try {
      browser.notifications.create({
        type: "basic",
        iconUrl: "/icon/128.png",
        title: `LDownload - ${title}`,
        message,
      });
    } catch (e) {
      console.warn("[LDownload] notify: failed to create notification:", e);
    }
  }
