  ...
  async main(ctx) {
    ...
    if (sniffingEnabled) {
      try {
        ...
        console.warn("[LDownload] Failed to inject fetch interceptor:", e);
      }
    }
    ...
    document.addEventListener("ldownload-resource-detected", handleFetchEvent);
    ctx.onInvalidated(() => {
      document.removeEventListener(
        "ldownload-resource-detected",
        handleFetchEvent,
      );
    });
    ...
    document.addEventListener("ldownload-dash-manifest", handleDashManifestEvent);
    ctx.onInvalidated(() => {
      document.removeEventListener(
        "ldownload-dash-manifest",
    ...
    // 立刻转发给 background，在浏览器发起 CDN GET 之前通知 LDownload。
    ...
    document.addEventListener("ldownload-preempt-download", handlePreemptEvent);
    ctx.onInvalidated(() =>
      document.removeEventListener(
        "ldownload-preempt-download",
        handlePreemptEvent,
      ),
    );
    ...
    // 改由 LDownload 接管。使用捕获阶段，早于页面自身的 click 处理器执行。
