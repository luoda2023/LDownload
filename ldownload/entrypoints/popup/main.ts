...
import { checkLDownloadAvailable } from '@/utils/download-dispatch';
...
async function refreshConnectionStatus(): Promise<void> {
  ...
  const available = await checkLDownloadAvailable().catch(() => false);
...
async function init() {
  const [, localState, settings] = await Promise.all([
    initI18n(),
    browser.storage.local.get(['theme', 'ldownload_dot_visible', 'stats']),
    loadSettings(),
  ]);
  ...
  protocolToggle.checked = settings.enableLdownloadProtocol === true;
  updateProtocolHint(settings.enableLdownloadProtocol === true);
  dotVisibleToggle.checked = localState?.['ldownload_dot_visible'] !== false;
...
dotVisibleToggle.addEventListener('change', async () => {
  await browser.storage.local.set({ ldownload_dot_visible: dotVisibleToggle.checked });
});
...
protocolToggle.addEventListener('change', async () => {
  await saveSettings({ enableLdownloadProtocol: protocolToggle.checked });
...
init().catch((e) => {
  console.error('[LDownload Popup] Init failed:', e);
});
