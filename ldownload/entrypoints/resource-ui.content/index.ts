...
const STORAGE_KEY = 'ldownload_dot_pos';
const DOT_VISIBLE_KEY = 'ldownload_dot_visible';
...
export default defineContentScript({
  ...
  async main(ctx) {
    console.log('[LDownload UI] starting');
    ...
    browser.storage.onChanged.addListener((changes, area) => {
      ...
      if (changes['ldownload_locale']) {
        const newLocale = changes['ldownload_locale'].newValue;
        if (newLocale) {
          setLocale(newLocale);
          refreshStaticTexts();
          render();
    ...
    function buildPanel(root: HTMLElement): void {
      panelEl = h('div', 'ldownload-panel');

      const header = h('div', 'panel-header');
      header.innerHTML = `
        ${svg(SVG_LOGO, 'logo')}
        <span class="title">LDownload</span>
    ...
    function buildFloatButton(root: HTMLElement): void {
      floatBtnEl = h('div', 'ldownload-float-btn');
    ...
    function buildQualityPicker(root: HTMLElement): void {
      qualityPickerEl = h('div', 'ldownload-quality-picker');
    ...
    function buildPreviewModal(root: HTMLElement): void {
      previewModalEl = h('div', 'ldownload-preview-modal');
      previewModalEl.innerHTML = `
        <div class="ldownload-preview-card">
