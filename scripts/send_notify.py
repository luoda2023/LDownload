...
# ── SMTP 配置 ────────────────────────────────────────────────────────────────
# 凭据一律来自环境变量，禁止硬编码：
#   LDOWNLOAD_SMTP_USER  发件邮箱
#   LDOWNLOAD_SMTP_PASS  SMTP 授权码
SMTP_HOST = os.environ.get("LDOWNLOAD_SMTP_HOST", "smtp.163.com")
SMTP_PORT = int(os.environ.get("LDOWNLOAD_SMTP_PORT", "465"))
SMTP_USER = os.environ.get("LDOWNLOAD_SMTP_USER", "")
SMTP_PASS = os.environ.get("LDOWNLOAD_SMTP_PASS", "")
SENDER_NAME = "LDownload"
...
def build_text(
    ...
    return f"""LDownload {platform} v{version} 正式发布！

你好！感谢订阅 LDownload {platform} 平台发布通知。
...
---
© 2025 LDownload · luoda2023 · https://dicad.cn
如有问题或建议：https://dicad.cn/feedback
"""
...
def build_message(
    ...
    subject = f"LDownload {platform} v{version} 正式发布 🎉"
...
def new_smtp_conn() -> smtplib.SMTP_SSL:
    ...
    if not SMTP_USER or not SMTP_PASS:
        print(
            "[错误] 未设置 LDOWNLOAD_SMTP_USER / LDOWNLOAD_SMTP_PASS 环境变量",
            file=sys.stderr,
        )
        sys.exit(1)
...
# ── 主流程 ───────────────────────────────────────────────────────────────────
def main() -> None:
    parser = argparse.ArgumentParser(description="LDownload 平台发布通知脚本")
    parser.add_argument(
        "--config",
