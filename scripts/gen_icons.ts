#!/usr/bin/env bun
/**
 * gen_icons.ts — 从 ldownload_logo.svg 生成全平台图标
 *
 * 用法:
 ...
 * 依赖: sharp (bun 全局已安装)
 *
 * 从 assets/logo/ldownload_logo.svg 生成以下全部图标:
 *
 *   assets/logo/
 *     ldownload_logo.png (600×600)
 *     ldownload_bolt.png (512×512, 内置备选应用图标「闪电」)
 *     logo.png (600×600)
 ...
 *
 *   ldownload/public/icon/
 *     {16,32,48,128}.png  {16,32,48,128}-disabled.png
 *     ldownload_logo.png (128×128)  ldownload_logo.svg (副本)
 *
 *   website/public/
...
const SVG_SRC = join(ROOT, "assets", "logo", "ldownload_logo.svg");
// 内置备选图标「闪电」源 SVG（可选 — 不存在时跳过生成）
const BOLT_SVG_SRC = join(ROOT, "assets", "logo", "ldownload_bolt.svg");
...
async function main() {
  console.log("🦅 LDownload 全平台图标生成器");
  ...
  console.log("📁 assets/logo/");
  {
    ...
    await saveFile("assets/logo/ldownload_logo.png", logo600);
    ...
    if (existsSync(BOLT_SVG_SRC)) {
      ...
      await saveFile("assets/logo/ldownload_bolt.png", bolt);
      totalCount += 1;
    }
  ...
  console.log("\n📁 ldownload/public/icon/");
  {
    ...
    for (const size of extSizes) {
      const buf = await getCachedPng(size);
      await saveFile(`ldownload/public/icon/${size}.png`, buf);
    }

    // disabled（灰度）图标
    for (const size of extSizes) {
      const buf = await renderDisabledPng(size);
      await saveFile(`ldownload/public/icon/${size}-disabled.png`, buf);
    }

    // 扩展 logo
    const extLogo = await getCachedPng(128);
    await saveFile("ldownload/public/icon/ldownload_logo.png", extLogo);

    // 复制 SVG 到扩展
    const svgContent = readFileSync(SVG_SRC);
    await saveFile("ldownload/public/icon/ldownload_logo.svg", svgContent);
