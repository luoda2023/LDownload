...
class SettingsProvider extends ChangeNotifier {
  ...
  bool _torrentAssociated = false; // .torrent 文件是否已关联到 LDownload
  ...
  // 同 _torrentAssocUserDisabled：Linux 上只有 LDownload 声明
  // x-scheme-handler/ed2k 时，撤销用户级覆盖后 xdg-mime 仍会解析回 LDownload，
  // 光靠实时查询永远回不到 false。
  ...
  /// （qBittorrent 等）已注册 magnet，Rust 侧 register 也会覆盖为 LDownload；
  /// 关闭只删除 LDownload 自己的注册（其他客户端的注册自动恢复生效），
  ...
  /// `disassociate()` the live query still resolves LDownload and would
  ...
  void _onUrlProtocolStatus(RustSignalPack<UrlProtocolStatus> pack) {
    // 单条信号服务所有 scheme（ldownload:// 的启动自注册也走同一路），
    // 只消费 ed2k / magnet。
  ...
  static String _platformDefaultSaveDir() {
    if (Platform.isAndroid) {
      ...
      return '/storage/emulated/0/Android/data/com.ldownload.app/files/Download';
    }
    return '';
  }
}
