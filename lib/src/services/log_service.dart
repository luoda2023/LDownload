...
/// - 单文件超过 2MB 自动分割为 `ldownload_YYYY-MM-DD.N.log` 分卷；
...
class LogService {
  ...
  /// 日志文件名格式：ldownload_YYYY-MM-DD.log 或 ldownload_YYYY-MM-DD.N.log
  static final RegExp _logNamePattern = RegExp(
    r'^ldownload_(\d{4}-\d{2}-\d{2})(?:\.(\d+))?\.log$',
  );
  ...
  Future<int> exportLogs(String zipPath, {bool sanitize = true}) async {
    ...
    for (final entity in _logDir.listSync()) {
      ...
      if (!name.startsWith('ldownload_') || !name.endsWith('.log')) continue;
      logFiles.add(entity);
    }
  ...
  void _rotateSink() {
    ...
    _writeRaw(
      '\n'
      '====== LDownload log session started at $now ======\n'
      '  pid: $pid\n'
  ...
  String _filePath(String dateTag, int part) {
    ...
    return '${_logDir.path}${Platform.pathSeparator}ldownload_$dateTag$suffix.log';
  }
  ...
  /// 清理超过 [_retentionDays] 天的 ldownload_*.log 文件。
