...
const _macWindowChannel = MethodChannel('com.ldownload/window');

/// 从托盘/最小化/关闭到托盘恢复主窗口并置于前台。
///
/// macOS：走原生 `com.ldownload/window` → AppDelegate.restoreMainWindow，
...
/// 统一经 `com.ldownload/window` 通道转发到
...
class TrayService with TrayListener {
  ...
  /// 应用退出回调 — 由外部（如 _LDownloadAppState）设置以实现优雅退出。
  ...
  Future<void> init() async {
    ...
    if (Platform.isWindows) {
      ...
      // 初始值使用系统亮度；启动后由 _LDownloadAppState 通过 setIsDark() 修正为 app 主题
    ...
    if (!Platform.isLinux) {
      await trayManager.setToolTip('LDownload');
    }
  ...
  /// 由外部（_LDownloadAppState）在应用主题或系统亮度变化时调用，
