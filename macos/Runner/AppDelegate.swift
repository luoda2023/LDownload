...
class AppDelegate: FlutterAppDelegate, UNUserNotificationCenterDelegate {
  override func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    //   -[LDownload.AppDelegate applicationDidFinishLaunching:]: unrecognized selector
  ...
  /// (MethodChannel `com.ldownload/window` → restore) 共用。
  ///
  ...
  // MARK: - ldownload:// URL scheme

  /// 冷启动时暂存的协议 URL：URL open 事件可能早于 Dart 侧
  /// `com.ldownload/single_instance` 的 handler 注册，先缓冲再重试投递。
  private var pendingUrls: [String] = []
  private var deliverRetryCount = 0

  /// 系统派发 `ldownload://` URL（CFBundleURLTypes 注册）：与 Windows 的
  ...
  private func deliverPendingUrls() {
    ...
    let channel = FlutterMethodChannel(
      name: "com.ldownload/single_instance",
