 ...
 *     - 已归档旧仓库 luoda2023/ldownload-archive 全量 release 下载：55,318
 ...
 *   } | null,  // LDownload Server（headless Web 版），无对应 release 时为 null
...
// ── 历史基础下载量 ──
// 已归档旧仓库 luoda2023/ldownload-archive（55,318）+ 已停用的 Cloudflare R2
...
export const GET: APIRoute = async ({ url }) => {
  ...
  try {
    ...
    // LDownload Server release：独立 server-v* release（headless Web 服务器）
    ...
    const serverRelease = pickRelease(
      appPool,
      serverRe,
      (a) => a.name.startsWith("LDownload-Server-"),
      includePrerelease,
    );

    // LDownload CLI release：独立 cli-v* release（命令行客户端 ldownload）
    ...
    const cliRelease = pickRelease(
      appPool,
      cliRe,
      (a) => a.name.startsWith("LDownload-CLI-"),
      includePrerelease,
    );

    // LDownload 移动端 release：独立 mobile-v* release（Android APK）
    ...
    const extensionVersion = extensionRelease
      ? /^extension-v(\d+\.\d+\.\d+)$/.exec(extensionRelease.tag_name)?.[1] ??
        /^LDownload-(\d+\.\d+\.\d+)-(?:chrome|extension)\.zip$/.exec(
          extensionAsset?.name ?? "",
        )?.[1] ??
        /^LDownload-(\d+\.\d+\.\d+)-firefox\.xpi$/.exec(
    ...
    // LDownload Server 资产（独立 server-v* release，命名：LDownload-Server-<ver>-<os>-<arch>.<ext>）
    const findServerAsset = (suffix: string) =>
      serverRelease?.assets.find(
        (a) =>
          a.name.startsWith("LDownload-Server-") && a.name.endsWith(suffix),
      );
    ...
    // LDownload CLI 资产（命名：LDownload-CLI-<ver>-<os>-<arch>.<ext>）
    const findCliAsset = (suffix: string) =>
      cliRelease?.assets.find(
        (a) => a.name.startsWith("LDownload-CLI-") && a.name.endsWith(suffix),
      );
    ...
    // 移动端 Android 资产（命名：LDownload-<ver>-android-<abi>.apk）
