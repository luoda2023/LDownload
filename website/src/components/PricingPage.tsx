...
const LOGIN_URL = "/api/auth/github?returnTo=/pricing";
const LOGOUT_URL = "/api/auth/logout?returnTo=/pricing";

const PLAN_CARDS: {
  key: Plan;
  nameKey: keyof Messages;
  descKey: keyof Messages;
  unitKey: keyof Messages;
...
}[] = [
  {
    ...
    accent: "#38bdf8",
    gradient: "from-brand-sky/[0.08] to-transparent",
  },
  {
    key: "subscription",
    nameKey: "pricing.subName",
    descKey: "pricing.subDesc",
    unitKey: "pricing.perMonth",
    featureKeys: ["pricing.subF1", "pricing.subF2", "pricing.subF3", "pricing.subF4"],
    accent: "#22d3ee",
    gradient: "from-brand-cyan/[0.08] to-transparent",
  },
];
    ...
    <section className="pt-24 sm:pt-32 pb-16 sm:pb-20">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-w-3xl mx-auto">
          {PLAN_CARDS.map((plan, i) => (
            <motion.div
              ...
              className="relative rounded-xl border border-dark-border overflow-hidden"
            >
              ...
              <div className="relative p-6 sm:p-8">
                ...
                <div className="mt-4 flex items-baseline gap-2">
                  <span
                    className="inline-flex items-center rounded-lg border border-dashed px-3 py-1 text-2xl font-bold tracking-tight"
                    style={{ color: plan.accent, borderColor: `${plan.accent}55` }}
                  >
                    {t("pricing.tbd")}
                  </span>
                  <span className="text-sm text-dark-text-muted">{t(plan.unitKey)}</span>
                </div>
                ...
                <ul className="mt-6 space-y-2.5">
                  {plan.featureKeys.map((fk) => (
                    <li key={fk} className="flex items-start gap-2.5 text-sm text-dark-text-secondary">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={plan.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 shrink-0">
                        <polyline points="20 6 9 17 4 12" />
                      </svg>
                      {t(fk)}
                    </li>
                  ))}
                  <li className="flex items-start gap-2.5 text-sm text-dark-text-muted">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={plan.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 shrink-0">
                      <path d="M12 3v2" />
                      <path d="M12 19v2" />
                      <path d="m5 5 1.5 1.5" />
        ...
        >
          ...
          <p className="mt-3 text-sm sm:text-base text-dark-text-secondary max-w-2xl mx-auto leading-relaxed">
                ...
                <div className="mt-8 rounded-xl border border-dark-border bg-dark-surface1/30 p-5 sm:p-6">
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-brand-sky to-brand-cyan px-5 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50 transition-opacity"
                    >
