...
const DOCKER_IMAGE = "ghcr.io/luoda2023/ldownload-server:latest";

const DOCKER_RUN_CMD = `docker run -d --name ldownload-server \\
  -p 17800:17800 \\
...
const SCOOP_SELFHOSTED_CMD = `scoop bucket add ldownload https://github.com/luoda2023/LDownload
...
interface ReleaseInfo {
  ...
  /** LDownload Server（headless Web 版）独立 release，无发布时为 null */
  server: {
    version: string;
    tag: string;
    assets: {
  ...
  /** LDownload CLI（命令行客户端）独立 release，无发布时为 null */
  cli: {
    version: string;
    tag: string;
    assets: {
  ...
  /** LDownload 移动端（Android）独立 release，无发布时为 null */
  mobile: {
    version: string;
    tag: string;
    assets: {
...
export default function DownloadSection() {
  ...
  useEffect(() => {
    ...
    const w = window as { __ldownloadPendingPlatform?: string };
    apply(w.__ldownloadPendingPlatform);
    delete w.__ldownloadPendingPlatform;
    const handler = (e: Event) => {
      apply((e as CustomEvent<string>).detail);
      (window as { __ldownloadPendingPlatform?: string }).__ldownloadPendingPlatform =
        undefined;
    };
    window.addEventListener("ldownload:select-platform", handler);
    return () =>
      window.removeEventListener("ldownload:select-platform", handler);
  }, []);
  ...
  const platforms: {
    ...
    iconImg?: string;
    iconImgFull?: boolean;
    /** 平台独立版本号（如 LDownload Server），缺省时用桌面客户端版本 */
    version?: string;
    setup: ReleaseAsset | null;
    portable: ReleaseAsset | null;
    setupLabel?: string;
          ...
          >
            <div className="relative rounded-xl border border-dark-border bg-dark-surface1 p-6 flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                {/* Chrome 官方商店按钮 */}
                <a
                  href="https://chromewebstore.google.com/detail/ldownload/meleenglfggcmcajknpeeeiobnpfmahc"
                  target="_blank"
                ...
                {/* Firefox 官方商店按钮 */}
                <a
                  href="https://addons.mozilla.org/zh-CN/firefox/addon/ldownload"
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                {/* Edge 官方商店按钮 */}
                <a
                  href="https://microsoftedge.microsoft.com/addons/detail/ldownload/nglkkjbogjghekbhhcnccnpfedjbdhhd"
                  target="_blank"
                  rel="noopener noreferrer"
