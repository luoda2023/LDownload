...
function inferAssetMeta(name: string): {
  ...
  // CLI（独立 cli-v* release，命名 LDownload-CLI-<ver>-<os>-<arch>.<ext>）
  if (lower.startsWith("ldownload-cli-")) {
  ...
  // LDownload Server（独立 server-v* release，命名 LDownload-Server-<ver>-<os>-<arch>.<ext>）
  if (lower.startsWith("ldownload-server-")) {
  ...
  if (lower.startsWith("luci-app-ldownload_") && lower.endsWith(".ipk")) {
    return {
      label: "NAS",
      sub: "OpenWrt LuCI",
      icon: <HardDrive className="w-3.5 h-3.5" />,
  ...
  if (lower.startsWith("ldownload-server_") && lower.endsWith(".ipk")) {
  ...
  // Android（独立 mobile-v* release，命名 LDownload-<ver>-android-<abi>.apk）
...
const LANG_MARKER_RE = /<!--\s*ldownload:lang:(zh|en)\s*-->/g;
