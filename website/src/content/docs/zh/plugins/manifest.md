...
| `minAppVersion` | 否 | string | 三段版本号。运行中的 LDownload 比它旧时，插件在加载阶段被跳过（记日志，不算错误）。 |
...
注意：同一插件如果还声明了 resolver，`onMetaProbed` 对它的任务永远不触发——带 resolver 的任务直接跳过元数据探测。你照订阅的话，LDownload 会记一条警告日志。
...
未知值会让整份 manifest 失败——这样不认识某权限的旧版 LDownload 会拒绝该插件，而不是悄悄忽略；新增权限时请一并抬高 `minAppVersion`。
