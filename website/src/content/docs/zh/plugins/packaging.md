...
LDownload 的市场是**一份数据格式，不是一个服务**——一份应用直接读取的索引，无后端、无账号。组成部分：

- **索引**——一份 Git 版本化的 JSON 文件，列出插件、版本和下载镜像。默认索引在 GitHub 的 `luoda2023/ldownload-plugin-index` 仓库；任何人都可以 fork 一份自己维护。用户可以在应用里添加自定义索引源。
- **内容寻址**——每个发布版本都记录 `contentHash = sha256(整个 .fxplug 文件)`。从任何镜像下载后，LDownload 重新计算哈希，不一致就拒绝。被攻破的镜像换不掉内容。
- **多镜像**——每个版本列出多个下载地址（raw.githubusercontent、jsDelivr、GitHub Releases……），LDownload 依次尝试。镜像 URL 必须是 `https`，且不能指向私网/环回/元数据地址。
