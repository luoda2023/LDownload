---
title: 命令行客户端
description: ldownload 命令行客户端 —— aria2c 风格的下载客户端，从终端或脚本驱动管理 API。
section: api
order: 2
sourceHash: "fa193ab09799"
---

LDownload 提供一个命令行客户端 `ldownload`，对标 `aria2c`。它有两种工作模式：

- **远程模式（默认）**：一个薄的 typed HTTP 客户端，运行在[管理 API](/docs/zh/api/overview/) 之上。绝大多数命令都与运行中的 LDownload 桌面应用或 [Headless 服务器](/docs/zh/headless-server/setup/) 通信（默认 `http://127.0.0.1:17800`，也可指向任意地址），因此它管理的正是你在应用里看到的同一批任务与队列。在这个角色下，可以把它理解为一个可脚本化的远程遥控器，与 `aria2c` 的 RPC 客户端模式相同。
...
# 二进制位于 target/release/ldownload（Windows 上为 ldownload.exe）
```
...
| 选项 | 环境变量 | 默认值 | 含义 |
|---|---|---|---|
| `--url <URL>` | `LDOWNLOAD_URL` | `http://127.0.0.1:17800` | LDownload 实例的基址。 |
| `--token <TOKEN>` | `LDOWNLOAD_TOKEN` | （无） | 管理 API token。除 `ping` 外每条命令都需要。 |
...
export LDOWNLOAD_URL="http://127.0.0.1:17800"   # 可选；这就是默认值
ldownload list
...
```bash
ldownload watch                 # 全部活动任务
ldownload watch <ID>            # 单个任务
ldownload watch --interval 2    # 每 2 秒刷新（默认 1）
```
...
```bash
ldownload config set token fxd_your_token_here   # 持久化 token
ldownload config set url http://192.168.1.10:17800
ldownload config set timeout 60

ldownload config get token        # 打印单个值
ldownload config list             # 列出全部键（未设置显示 "(unset)"）
ldownload --json config list      # 机器可读输出
ldownload config unset token      # 清除单个值
ldownload config path             # 打印配置文件路径
```

配置文件是平台配置目录下的 `cli.toml`（Windows `%APPDATA%\LDownload\config\`、Linux `$XDG_CONFIG_HOME/ldownload/`、macOS `~/Library/Application Support/dev.ldownload/`）。存储的 `token` 为明文；在 Unix 上文件以 `0600` 权限创建（仅属主可读写）。这里设置的值，在具体某次调用时会被显式 `--flag` 或对应环境变量覆盖。
...
CLI 有两种角色：远程模式驱动同一套管理 API，独立模式（`add --local`）内嵌引擎脱机下载。若你已有面向 aria2 的工具链，[aria2 兼容 RPC 端点](/docs/zh/api/overview/#curl-示例)可能更合适；面向 AI 客户端则用 [MCP](/docs/zh/api/overview/#mcpmodel-context-protocol)。远程模式与这些入口操作的是同一批任务与队列。Metalink、XML-RPC、以及 aria2 的会话保存/恢复刻意未实现 —— LDownload 的 SQLite 存储已在重启间持久化了一切。
