---
title: API Overview
description: The LDownload HTTP API — route groups, authentication, and how it relates to the headless server.
section: api
order: 1
---

LDownload ships a small HTTP API — used by browser extensions, userscripts, aria2 clients, and automation — built into two places:
...
| Script takeover | `POST /download`, `POST /download/batch` | `local_server_takeover_enabled` (default on) | `X-LDownload-Client` header required, plus an optional token |
...
|---|---|
| Script takeover | `X-LDownload-Token` header (only if a token is configured — empty token means the group is unauthenticated). The `X-LDownload-Client` header is always required regardless of token, as a CORS-based gate against arbitrary web pages. |
| aria2-compatible RPC | `X-LDownload-Token` header, **or** aria2's own convention of passing `token:xxx` as `params[0]` in the JSON-RPC call. |
| Management API (`/api/v1/*`) | `Authorization: Bearer <token>` **or** `X-LDownload-Token` header. If no token is configured, every management request is rejected (403) — this group cannot run unauthenticated. |
...
LDownload speaks [MCP](https://modelcontextprotocol.io) over HTTP, so AI clients (Claude Desktop, Cursor, Cline, and any MCP-capable agent) can drive downloads in natural language. It's a single endpoint, `POST /mcp`, protected by the same token as the management API.

MCP is JSON-RPC 2.0 over one HTTP endpoint (not REST) — every operation is one POST to `/mcp`, distinguished by the `method` in the body, using the stateless subset of the Streamable HTTP transport: requests get an `application/json` response, notifications get `202 Accepted`, and no session id is tracked. Authenticate with `Authorization: Bearer <token>` (or `X-LDownload-Token`); the spec permits a static bearer token for internal deployments in place of OAuth 2.1.
...
## The ldownload:// URL protocol

Alongside the HTTP API, LDownload registers a custom URL protocol that any web page, script, or third-party app can use to hand a download off — no local HTTP call required:

```text
ldownload://download?url=<percent-encoded URL>&filename=<optional name>
```

- `url` — required. The address to download, percent-encoded (`http`/`https`/`ftp` direct links or a `magnet:` link). A `ldownload://` URL with a missing or empty `url` parameter is silently ignored.
...
- **Desktop (Windows, macOS, Linux)** — the app registers the protocol handler (Windows registry on every startup; a `CFBundleURLTypes` declaration on macOS; an `x-scheme-handler` entry in the `.desktop` file on Linux). Opening a `ldownload://` URL launches the app (or forwards to the already-running instance) and routes the request into the same external-download flow as browser-extension requests: a quick-download confirmation by default, silent task creation if the user enabled no-prompt downloads. On Android and in restricted desktop environments, the browser extension itself can deliver through this protocol — see [the ldownload:// protocol mode](/docs/en/browser-extension/usage/).
...
```html
<a href="ldownload://download?url=https%3A%2F%2Fexample.com%2Ffile.zip&filename=file.zip">
  Download with LDownload
</a>
```
