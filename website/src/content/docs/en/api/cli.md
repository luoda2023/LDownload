...
description: The ldownload CLI — an aria2c-style download client that drives the management API from your shell or scripts.
section: api
order: 2
---

LDownload ships a command-line client, the `ldownload` binary, modelled on `aria2c`. It works in two modes:

- **Remote mode (default)**: a thin, typed HTTP client over the [management API](/docs/en/api/overview/). Most commands talk to a running LDownload desktop app or [headless server](/docs/en/headless-server/setup/) over `http://127.0.0.1:17800` (or wherever you point it), so it manages exactly the same tasks and queues you'd see in the app. In this role it's a scriptable remote control, the same part `aria2c`'s RPC client mode plays.
...
```bash
cargo build -p ldown_cli --release
# binary at target/release/ldownload (ldownload.exe on Windows)
```

Or run it straight through Cargo during development:

```bash
cargo run -p ldown_cli -- ping
```
...
|---|---|---|---|
| `--url <URL>` | `LDOWNLOAD_URL` | `http://127.0.0.1:17800` | Base address of the LDownload instance. |
| `--token <TOKEN>` | `LDOWNLOAD_TOKEN` | (none) | Management API token. Required for every command except `ping`. |
...
export LDOWNLOAD_TOKEN="fxd_your_token_here"
export LDOWNLOAD_URL="http://127.0.0.1:17800"   # optional; this is the default
ldownload list

# Option B: persist once, no exports needed afterwards
ldownload config set token fxd_your_token_here
ldownload list
```
...
ldownload add https://example.com/file.zip

# Several URLs at once
ldownload add https://example.com/a.zip https://example.com/b.zip

# Read URLs from a file (one per line; blank lines and lines starting with # are ignored)
ldownload add -i urls.txt

# Read URLs from stdin
cat urls.txt | ldownload add -i -
```
...
```bash
# Fully offline download, independent of any running LDownload instance
ldownload add https://example.com/file.zip --local
```
...
```bash
ldownload list
ldownload list --status downloading
ldownload --json list          # scriptable output
```
...
```bash
ldownload status 42a14870-9276-4ea2-84ea-eb75ae497766
ldownload pause  42a14870-9276-4ea2-84ea-eb75ae497766
ldownload resume 42a14870-9276-4ea2-84ea-eb75ae497766

# delete the task record only
ldownload rm 42a14870-9276-4ea2-84ea-eb75ae497766
# delete the record AND the file on disk
ldownload rm 42a14870-9276-4ea2-84ea-eb75ae497766 --delete-files
```
...
```bash
ldownload pause-all
ldownload resume-all
ldownload queue           # list named queues
```
...
```bash
ldownload watch                 # all active tasks
ldownload watch <ID>            # one task
ldownload watch --interval 2    # refresh every 2 seconds (default 1)
```
...
```bash
ldownload config set token fxd_your_token_here   # persist the token
ldownload config set url http://192.168.1.10:17800
ldownload config set timeout 60

ldownload config get token        # print one value
ldownload config list             # show every key (unset keys shown as "(unset)")
ldownload --json config list      # machine-readable
ldownload config unset token      # clear one value
ldownload config path             # print the config file location
```

The file is `cli.toml` under the platform config directory (`%APPDATA%\LDownload\config\` on Windows, `$XDG_CONFIG_HOME/ldownload/` on Linux, `~/Library/Application Support/dev.ldownload/` on macOS). The stored `token` is plain text; on Unix the file is created with `0600` permissions (owner-only). A value set here is overridden by an explicit `--flag` or the matching environment variable on any given invocation.
...
| `5` | Network error (couldn't connect — is LDownload running?). |
