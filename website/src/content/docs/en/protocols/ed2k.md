...
LDownload includes a built-in eD2K (eDonkey2000) client — paste an `ed2k://` link into the New Download dialog and it downloads like any other task. The client is **download-only**: it fetches files from the network but does not share or upload, so no library management or upload quotas are involved.

## ed2k:// links

LDownload accepts standard file links of the form:
...
LDownload looks for peers that have the file through two complementary channels, both enabled by default:
...
If no sources are found, LDownload retries a few times about a minute apart before marking the task as failed — rare files can take several attempts to locate.
...
**Does LDownload upload while downloading?** No. The client is strictly download-only; it never shares files back to the network.
