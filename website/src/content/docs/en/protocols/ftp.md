...
Leave off the `user:pass@` part entirely for an anonymous login (LDownload defaults to the standard `anonymous` / `anonymous@` credentials). If your password itself contains an `@`, percent-encode it as `%40` so it isn't mistaken for the host separator.
...
**My FTP download only uses one connection even though I picked more threads.** Either the file is 1 MB or smaller, or LDownload's REST-honouring probe determined the server doesn't correctly support resuming multi-segment transfers — both fall back to a single connection to guarantee a correct file over raw speed.
...
**Anonymous FTP isn't working.** Double-check the URL has no stray `user:pass@` left in it — if it's present but empty or malformed, LDownload won't fall back to anonymous login automatically.
