...
LDownload's market is **a data format, not a service** — an index the app reads directly, with no backend and no account. The pieces:

- **An index** — a Git-versioned JSON file listing plugins, versions and download mirrors. The default index lives at `luoda2023/ldownload-plugin-index` on GitHub; anyone can fork it and run their own. Users can add custom index sources in the app.
- **Content addressing** — every published version records `contentHash = sha256(<the .fxplug file>)`. After downloading from any mirror, LDownload recomputes the hash and rejects a mismatch. A compromised mirror cannot swap the payload.
- **Multiple mirrors** — each version lists several download URLs (raw.githubusercontent, jsDelivr, GitHub Releases…). LDownload tries them in order. Mirror URLs must be `https` and must not point at private/loopback/metadata addresses.
