...
| `minAppVersion` | no | string | Three-part version. If the running LDownload is older, the plugin is skipped at load (logged, not an error). |
...
Caveat: if the same plugin also declares a resolver, `onMetaProbed` never fires for its tasks — resolver tasks skip the metadata probe entirely. LDownload logs a warning if you subscribe to it anyway.
...
Unknown values fail the whole manifest, so an older LDownload that doesn't know a permission rejects the plugin rather than silently ignoring it — pair a new permission with a `minAppVersion` bump.
