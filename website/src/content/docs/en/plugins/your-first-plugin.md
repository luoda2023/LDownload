...
This walkthrough builds a resolver plugin from scratch: it intercepts links from a fictional file host, calls the host's API to get the real download link, and hands that link to LDownload. When you're done you'll know the full edit–test loop.
...
async function resolve(ctx) {
  ...
  if (!id) return null; // null/undefined = pass through, LDownload downloads ctx.url as-is
...
- `ephemeral: true` tells LDownload the link is one-shot or anti-hotlinked, so it skips the extra HEAD probe that could burn the link. If your host's links are stable, leave it out — the probe gets you better resume integrity (ETag checks).
- `rangeSupported: true` promises the host serves HTTP Range requests. Without a probe LDownload would otherwise start conservatively with a single connection; with the promise it plans multi-segment right away. Only declare it when you know the host supports Range — a false promise can waste quota-limited links.
...
2. Add (or resume) a matching download in LDownload.
...
- **Logs** — `flux.logger.*` and `console.log` both go to LDownload's log file (`logs/ldownload_YYYY-MM-DD.log` next to the app on Windows, `~/.local/share/ldownload/logs/` on Linux). Log lines are truncated at 4 KB.
