...
description: Every setting in LDownload, grouped by the categories in the Settings sidebar, with defaults.
...
|---|---|---|
| Launch at Startup | Automatically run LDownload when the system starts. | Off |
...
| Associate .torrent Files | Make LDownload the default handler for `.torrent` files. LDownload prompts once on first launch if this hasn't been set. | Off |
...
To use a manager that is **not** your system default — or to highlight the exact file inside a third-party manager — fill in the **File manager command** template above. One command covers both "Show in Folder" (a downloaded file) and "Open Folder" (a directory); LDownload fills the placeholders according to the case. Placeholders are **already shell-quoted, so don't add your own quotes** around them — but the executable path still needs quotes if it contains spaces:
...
A configured template always takes priority over auto-detection; if it fails to launch, LDownload falls back to the platform default.

## BitTorrent

| Setting | Purpose | Default |
|---|---|---|
| Listen Port Range | The port range LDownload listens on for incoming BT connections. | 6881–6891 |
...
Some BT changes require restarting the BT engine to take effect (LDownload shows a reminder in this category). DHT and UPnP are always enabled internally and have no separate toggle. Proxy settings (below) don't apply to BitTorrent downloads.
...
| Browser Script Takeover | Lets the LDownload userscript take over browser downloads at `http://127.0.0.1:<port>`. Includes a button to copy the userscript. | On |
...
| Max Log Size | Total size cap for LDownload's log files (5/10/20/50/100 MB); the oldest logs are cleaned automatically once the cap is exceeded. | 10 MB |
