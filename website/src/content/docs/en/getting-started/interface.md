...
description: A tour of LDownload's three-panel layout, sidebar, top bar, task list, detail panel, status bar, shortcuts, and tray.
section: getting-started
order: 3
---

LDownload's main window is a three-panel layout: a resizable **sidebar** on the left, the **task list** in the center, and an optional **detail panel** on the right that opens when you select a task.
...
- **Progress** — a large percentage readout, a segmented progress bar, and (once segment data exists) an IDM-style pixel grid where each cell reflects how full that byte range is, plus a legend listing each segment's number and percentage. When LDownload proactively or reactively splits a slow segment, a running split count and the most recent split's byte range are shown alongside the progress bar.
...
LDownload keeps a tray icon running while the window is minimized or closed to tray. Right-click it for:
