...
This walks through the full flow of creating a download in LDownload, controlling it while it runs, and organizing tasks with queues and speed limits. See [Installation](/docs/en/getting-started/installation/) if you haven't set up LDownload yet, and [Interface Overview](/docs/en/getting-started/interface/) for a tour of every panel mentioned here.
...
| Field | What it does |
|---|---|
| **Download URL** | A multi-line box — paste one URL per line to queue a batch download (magnet and `ed2k://` links work too), or a single URL for one task. LDownload shows a live count of parsed URLs. Use **Open .torrent file** to pick a local `.torrent`, or **Import TXT file** to load a list of URLs from a text file. |
...
| **Threads** | Segments to split the download into: **Auto** (LDownload picks based on file size and CPU count), a fixed preset (4/8/16/32/64), or a custom value from 1–256. Hidden for magnet links and `.torrent` files, since BitTorrent manages its own connections. |
...
| **Hash Verification** | Pick an algorithm (MD5/SHA-1/SHA-256/SHA-512) and paste the expected hash. LDownload verifies the file after download and flags a mismatch. Leave blank to skip. |
...
When the clock crosses a scheduled time on an active day, the queue starts or stops exactly as if you'd clicked the button; each edge fires at most once per day, and an edge that already passed earlier the same day is caught up when LDownload launches. Queues with an active schedule show a small alarm-clock icon in the sidebar.
