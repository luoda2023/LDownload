...
const VOTES_ISSUE_TITLE = "[LDownload] Feature Vote Records";
...
/** 从旧 body 提取描述与 meta（<!-- ldownload:feature-meta {...} -->） */
function parseOldBody(body) {
  const raw = body ?? "";
  let meta = null;
  const m = raw.match(/<!--\s*ldownload:feature-meta\s*([\s\S]*?)-->/);
  if (m) {
    try {
      meta = JSON.parse(m[1]);
    } catch {
  ...
  const description = raw
    .replace(/<!--\s*ldownload:feature-meta[\s\S]*?-->/g, "")
    .trim();
