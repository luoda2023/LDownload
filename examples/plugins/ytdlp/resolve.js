// LDownload 插件：yt-dlp 通用视频解析（classic script，入口挂 globalThis）。
//
// 原理：调用 LDownload 自带的 yt-dlp 组件（flux.ytdlp），以 `-J`（--dump-single-json）
...
// 直通 yt-dlp 高级能力；LDownload bridge 仍会拒绝危险开关（--exec 等）。播放列表 URL
// 可经「播放列表条目」设置项选择下载第几条（--playlist-items 单值）。
//
// yt-dlp 组件可在 App「组件」页安装；LDownload 会自动注入 `--ffmpeg-location`（合并/
// remux 依赖 ffmpeg，插件自带的 --ffmpeg-location 会被 bridge 拒绝）。
//
// 返回值约定（ResolveResult）：
//   url / audioUrl / fileName / totalBytes / extraHeaders / ephemeral / rangeSupported
//   （详见各字段回填处注释）。
//   variants（可选，画质/格式多选项数组；非空时 LDownload 弹框让用户选择，60s 超时或
...
//                 时传 ''），避免 LDownload 收敛逻辑遗留上一档的音频直链
...
//   totalBytes 逐一相同，故旧版 LDownload（无 variants 支持）行为不回退；其后附加
...
// 的值，支持 \" 转义。空串 → 空数组。LDownload bridge 会对危险开关二次拦截。
...
// fileName / totalBytes 恒显式赋值（未知传 ''/0，而非省略字段），保证 LDownload
...
// info.formats 缺失/为空时仅返回单元素数组（LDownload 按 variants.len()<=1 跳过弹框）。
...
globalThis.resolve = async (ctx) => {
  ...
  // 缺运行时则 YouTube 格式 URL 缺失、只剩 storyboard（下不了）。LDownload 自动注入的
  ...
  // 附加参数（高级）：追加到命令末尾（URL 之前）。LDownload bridge 二次拦截危险开关。
