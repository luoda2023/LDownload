// ==UserScript==
// @name              LDownload 下载接管 / LDownload Download Capture
// @name:zh-CN        LDownload 下载接管
// @namespace         https://github.com/luoda2023/LDownload
// @version           1.0.0
// @description       拦截浏览器下载与流媒体资源，一键发送到 LDownload 桌面下载器（通过本地 RPC）。Intercept downloads & media, send them to the LDownload desktop app via local RPC.
// @description:zh-CN 拦截浏览器下载与流媒体资源（HLS/DASH/视频/音频/压缩包/安装包等），一键发送到 LDownload 桌面下载器。
// @author            luoda2023
// @license           MIT
// @match             *://*/*
...
// @noframes
// @homepageURL       https://github.com/luoda2023/LDownload
// @supportURL        https://github.com/luoda2023/LDownload/issues
// ==/UserScript==
 ...
 *   3. 通过 GM_xmlhttpRequest POST 到 LDownload 的本地 HTTP 接管服务
 *      （默认 http://127.0.0.1:17800/download），由桌面端弹出确认框后下载。
 *
 * 与浏览器扩展的能力差异（务必知悉）
 * ----------------------------------
 *   - 无法拦截「浏览器内核直接发起」的下载（非页面 JS 触发的、点击后直接由
 *     Content-Disposition 触发的下载）——这类请在 LDownload 浏览器扩展中完成；
 ...
 *   - 每个请求携带 X-LDownload-Client 头（LDownload 据此拦截恶意网页的跨域伪造请求）；
 *   - 可在菜单里设置 Token（与 LDownload 设置页一致）做额外鉴权；
 *   - 最终所有下载都会在 LDownload 弹出确认框，不会静默下载。
 *
 * 配置（点击油猴菜单 → LDownload ...）
 * -----------------------------------
...
(function () {
  ...
  if (W.__ldownload_userscript__) return;
  W.__ldownload_userscript__ = true;
  ...
  function authHeaders() {
    const h = {
      ...
      'X-LDownload-Client': 'userscript',
    };
    const tk = CFG.token;
    if (tk) h['X-LDownload-Token'] = tk;
    return h;
  }
  ...
  // 把单个下载请求发给 LDownload。返回 true 表示已被接受。
  async function sendDownload(payload) {
    try {
      ...
      if (!ok) console.warn('[LDownload] send failed:', r.status, r.responseText);
      return ok;
    } catch (e) {
      console.warn('[LDownload] send error:', e);
      return false;
    }
  }

  // 批量下载（单次请求，url 由 LDownload 端按换行拆分，用户只需确认一次）。
  async function sendBatch(urls, shared) {
    ...
    try {
      ...
      console.warn('[LDownload] batch error:', e);
      return false;
    }
  }
  ...
  async function takeover(url, opts) {
    ...
    if (ok) {
      toast(`已发送到 LDownload：${opts.filename || filenameOf(url) || url}`);
    } else {
      // 失败：确认 App 是否在线，决定是否回退浏览器下载。
      const alive = await ping();
      if (alive) {
        toast('LDownload 收到请求（响应异常），请查看桌面端', true);
      } else {
        toast('LDownload 未运行，已交回浏览器下载', true);
  ...
  function browserFallback(url, filename) {
    ...
    try {
      ...
      a.setAttribute('data-ldownload-skip', '1');
  ...
  document.addEventListener('click', function (ev) {
    ...
    const a = ev.target && ev.target.closest
      ...
      : null;
    if (!a) return;
    if (a.hasAttribute('data-ldownload-skip')) return;
  ...
  try {
    ...
    W.HTMLAnchorElement.prototype.click = function () {
      try {
        if (CFG.enabled && !this.hasAttribute('data-ldownload-skip')) {
  ...
  function buildUI() {
    ...
    host.id = 'ldownload-userscript-root';
  ...
  async function sendAllSniffed() {
    ...
    toast(ok ? `已发送 ${urls.length} 个资源到 LDownload` : '发送失败，请确认 LDownload 在运行', !ok);
  }
  ...
  async function downloadAllLinks() {
    ...
    const ok = await sendBatch(urls);
    toast(ok ? `已发送本页 ${urls.length} 个链接到 LDownload` : '发送失败，请确认 LDownload 在运行', !ok);
  }
  ...
  function toast(msg, warn) {
    try {
      ...
      try { GM_notification({ text: msg, title: 'LDownload', timeout: 3000 }); } catch (__) { /* */ }
    }
  }
  ...
  function registerMenu() {
    ...
    add(`🔌 设置端口（当前 ${CFG.port}）`, () => {
      const p = prompt('LDownload RPC 端口（与设置页一致，默认 17800）：', String(CFG.port));
      if (p !== null) {
    ...
    add('🔑 设置 Token（可选）', () => {
      const t = prompt('LDownload RPC 授权密钥（在 LDownload 设置页生成，可留空）：', CFG.token);
      if (t !== null) { CFG.token = t.trim(); toast('Token 已保存'); }
    });
    add('🩺 测试连接', async () => {
      toast('正在测试…');
      const ok = await ping();
      toast(ok ? `已连接 LDownload（${base()}）` : `无法连接 ${base()}，请确认 LDownload 已启动且 RPC 服务已开启`, !ok);
    });
  }
  ...
  console.log('[LDownload] userscript loaded; target =', base());
})();
