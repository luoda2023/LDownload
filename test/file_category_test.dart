...
import 'package:ldownload/src/models/custom_category.dart';
...
void main() {
  ...
  group('CustomCategory.defaultCategories', () {
    test('内置 program 分类位于 archive 之前且匹配安装包', () {
      ...
      expect(program.matches('LDownload-0.1.58-windows-x64-setup.exe'), isTrue);
      expect(program.matches('LDownload-0.1.58-macos-arm64.dmg'), isTrue);
      expect(program.matches('ldownload_0.1.58_amd64.deb'), isTrue);
