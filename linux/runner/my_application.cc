...
struct _MyApplication {
  ...
  FlView* view;
  // Native controller for the com.ldownload/floating_ball channel (S3.4/A6).
...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "LDownload");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "LDownload");
  }
...
//    Send the file URIs to Dart via the com.ldownload/single_instance
