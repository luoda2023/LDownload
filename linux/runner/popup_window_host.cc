...
struct _PopupWindowHost {
  // ldownload/popup_host 通道，注册在主引擎 messenger 上，本结构体持有引用。
  ...
  FlView* popup_view;
  // ldownload/popup_child 通道，注册在弹窗引擎 messenger 上，本结构体持有
...
// =============================================================================
// 弹窗引擎通道 ldownload/popup_child 处理器
// =============================================================================
...
static void ensure_popup_window(PopupWindowHost* self) {
  ...
  //    聚焦（可能只弹"LDownload 已就绪"通知）——用户需点击一次弹窗才能输
  ...
  gtk_window_set_title(window, "LDownload Quick Download");
  ...
  // 生能力都经下面这条 ldownload/popup_child 通道由本文件提供，不加载任何
  // 生成的插件 registrant，也不触碰 Rust。
  g_autoptr(FlStandardMethodCodec) codec = fl_standard_method_codec_new();
  self->child_channel = fl_method_channel_new(
      fl_engine_get_binary_messenger(fl_view_get_engine(view)),
      "ldownload/popup_child", FL_METHOD_CODEC(codec));
...
// =============================================================================
// 主引擎通道 ldownload/popup_host 处理器
// =============================================================================
...
PopupWindowHost* popup_window_host_new(FlBinaryMessenger* main_messenger) {
  ...
  self->host_channel = fl_method_channel_new(
      main_messenger, "ldownload/popup_host", FL_METHOD_CODEC(codec));
