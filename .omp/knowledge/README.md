# LDownload internals · 索引 · 架构图 · 目录树
...
- **平台矩阵（已发布）**: Windows / macOS / Linux 桌面 App、Android App、headless Web 服务器（Docker/群晖/QNAP/OpenWrt/Unraid/CasaOS）、CLI（`ldownload`）、浏览器扩展、用户脚本。iOS 代码存在但无发布 job。
...
**一个引擎，多个宿主，多个客户端。** 所有下载逻辑集中在 `ldown_engine`（`native/engine`，零 FFI/零 rinf 依赖），通过**三个引擎自有 trait** 与外界解耦：
...
flowchart TB
  ...
  api[ldown_api<br/>ApiHost 契约 + HTTP 面]
  eng[ldown_engine<br/>协议/分段/DB/队列/组/插件]
  clients --> api
  api --> hub
  api --> srv
  hub --> eng
...
**要点**：
- `ldown_api` 只依赖 `&dyn ApiHost`，不碰引擎——同一套 HTTP 面（脚本接管 + aria2 JSON-RPC（POST 与 WS）+ MCP + `/api/v1` 管理 + OpenAPI）可服务任意宿主。
...
│   ├── engine/         `ldown_engine`：下载引擎（零 FFI）——核心，见 `engine.md`「下载引擎」
│   ├── api/            `ldown_api`：ApiHost 契约 + HTTP 面（零 rinf）——见 `hosts-and-api.md`「HTTP API」
│   ├── server/         `ldown_server`：headless Web 服务器——见 `hosts-and-api.md`「Headless 服务器」
│   ├── hub/            rinf FFI 适配层（唯一碰 rinf）——见 `hosts-and-api.md`「宿主与客户端 crate」
│   ├── cli/            `ldown_cli`：二进制 `ldownload`——见 `hosts-and-api.md`「宿主与客户端 crate」
│   ├── nmh/            Native Messaging Host 中继二进制
│   └── ldown_updater/  独立自更新 helper 二进制（hub 拉起）
├── web/                Web SPA（React 19 + TanStack + Tailwind v4，bun）——见 `clients.md`「Web SPA」
├── website/            官网（Astro SSR + 内容集文档系统）——见 `clients.md`「官网」
├── ldownload/           WXT 浏览器扩展（Chrome/Firefox MV3）——见 `clients.md`「浏览器扩展与用户脚本」
