...
OutputBaseFilename=LDownload-{#MyAppVersion}-windows-{#MyAppArch}-setup
...
english.TorrentAssoc=Associate .torrent files with LDownload
chinesesimplified.TorrentAssoc=将 .torrent 文件关联到 LDownload
...
Root: HKCU; Subkey: "Software\Classes\.torrent"; ValueType: string; ValueData: "LDownload.TorrentFile"; Flags: uninsdeletekey; Tasks: torrentassoc
Root: HKCU; Subkey: "Software\Classes\LDownload.TorrentFile"; ValueType: string; ValueData: "BitTorrent File"; Flags: uninsdeletekey; Tasks: torrentassoc
Root: HKCU; Subkey: "Software\Classes\LDownload.TorrentFile\DefaultIcon"; ValueType: string; ValueData: """{app}\{#MyAppExeName}"",0"; Flags: uninsdeletekey; Tasks: torrentassoc
Root: HKCU; Subkey: "Software\Classes\LDownload.TorrentFile\shell\open\command"; ValueType: string; ValueData: """{app}\{#MyAppExeName}"" ""%1"""; Flags: uninsdeletekey; Tasks: torrentassoc

[UninstallDelete]
; 删除 KvStore 落盘文件（含匿名统计设备 ID / 首装标记 / 窗口状态等本地偏好）。
; 语义：卸载后重装 = 生成新设备 ID = 统计为新安装；升级/覆盖安装不触发本节，ID 保留。
; 路径 = shared_preferences_windows：%APPDATA%\<CompanyName>\<ProductName>（Runner.rc 均为 LDownload）。
Type: files; Name: "{userappdata}\LDownload\LDownload\shared_preferences.json"
Type: dirifempty; Name: "{userappdata}\LDownload\LDownload"
Type: dirifempty; Name: "{userappdata}\LDownload"
...
Type: files; Name: "{app}\com.ldownload.nmh.json"
Type: files; Name: "{app}\com.ldownload.nmh.firefox.json"
...
begin
  Result := '';
  { Force-kill ldownload.exe as a fallback in case Restart Manager fails }
...
{ Remove a URL scheme handler (ldownload:// / ed2k:// / magnet:) registered at runtime by
...
begin
  if not RegQueryStringValue(HKCU, 'Software\Classes\LDownload.TorrentFile\shell\open\command', '', Command) then
    Exit;
  RegisteredExe := ExtractQuotedExe(Command);
  AppExe := ExpandConstant('{app}\{#MyAppExeName}');
  ...
  if RegQueryStringValue(HKCU, 'Software\Classes\.torrent', '', ProgId)
    and (CompareText(ProgId, 'LDownload.TorrentFile') = 0) then
    RegDeleteKeyIncludingSubkeys(HKCU, 'Software\Classes\.torrent');
  RegDeleteKeyIncludingSubkeys(HKCU, 'Software\Classes\LDownload.TorrentFile');
end;

{ Remove the autostart Run value written at runtime by the launch_at_startup
  plugin (lib/main.dart — value name "LDownload", data `"<exe>" --silentStart`).
...
begin
  ...
  begin
    { Chrome/Edge/Firefox Native Messaging Host registrations written at
      ...
      so the standard uninstall never removes them. `com.ldownload.nmh` is
      LDownload-specific, safe to remove unconditionally. }
    RegDeleteKeyIncludingSubkeys(HKCU, 'Software\Google\Chrome\NativeMessagingHosts\com.ldownload.nmh');
    RegDeleteKeyIncludingSubkeys(HKCU, 'Software\Microsoft\Edge\NativeMessagingHosts\com.ldownload.nmh');
    RegDeleteKeyIncludingSubkeys(HKCU, 'Software\Mozilla\NativeMessagingHosts\com.ldownload.nmh');

    { ldownload:// / ed2k:// / magnet: URL protocol handlers — same gap as above. }
    RemoveProtocolHandler('ldownload');
