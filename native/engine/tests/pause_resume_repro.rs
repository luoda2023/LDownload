...
//!
//! 运行：
//!   cargo test -p ldown_engine --test pause_resume_repro -- --ignored --nocapture
...
async fn run_scenario(
    ...
    let work_dir =
        std::env::temp_dir().join(format!("ldownload_pr_{}_{}", name, std::process::id()));
...
async fn run_change_segments(
    ...
    let work_dir =
        std::env::temp_dir().join(format!("ldownload_cs_{}_{}", name, std::process::id()));
    let _ = tokio::fs::remove_dir_all(&work_dir).await;
    tokio::fs::create_dir_all(&work_dir).await.unwrap();
