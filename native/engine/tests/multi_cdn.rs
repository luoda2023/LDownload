...
use ldown_engine::cdn::{ClientTemplate, NodePool};
use ldown_engine::db::Db;
use ldown_engine::downloader::{ProgressUpdate, RequestSpec, build_client};
use ldown_engine::events::{EngineEvent, EventSink};
use ldown_engine::proxy_config::ProxyConfig;
use ldown_engine::segment_coordinator::{ReportScope, run_coordinated_download};
...
fn work_dir(tag: &str) -> std::path::PathBuf {
    let d = std::env::temp_dir().join(format!("ldownload_mcdn_{}_{}", tag, std::process::id()));
    let _ = std::fs::remove_dir_all(&d);
    std::fs::create_dir_all(&d).expect("create work dir");
    d
}

const TEST_UA: &str = "LDownloadMultiCdnTest/1.0";
...
async fn run_with_pool(
    ...
    Result<i64, ldown_engine::downloader::DownloadError>,
    std::path::PathBuf,
) {
