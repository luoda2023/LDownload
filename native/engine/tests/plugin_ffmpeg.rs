...
use ldown_engine::plugin::runtime::FfmpegSpec;
...
fn unique_dir(tag: &str) -> PathBuf {
    ...
    d.push(format!("ldownload_ff_{}_{}_{}", tag, std::process::id(), n));
...
async fn make_bridge(data_dir: &Path) -> EngineBridge {
    ...
    // 测试用真实 ffmpeg：`LDOWNLOAD_TEST_FFMPEG=<绝对路径>` 时经 config 手动指定，
    // 使 resolve_ffmpeg 命中（CI/本机无系统 ffmpeg 时的确定性执行入口）。
    if let Ok(p) = std::env::var("LDOWNLOAD_TEST_FFMPEG") {
        db.set_config(ldown_engine::components::CONFIG_FFMPEG_PATH, &p)
            .await
