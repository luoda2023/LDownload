...
//! 正确解压 + manifest 校验。通过环境变量 `LDOWNLOAD_TEST_FXPLUG` 指向 .fxplug 路径运行：
//!
//! ```text
//! LDOWNLOAD_TEST_FXPLUG=.../echo-rewriter-1.0.0.fxplug \
//!   cargo test -p ldown_engine --features plugins --test fxplug_install
//! ```
...
use ldown_engine::plugin::install;

#[test]
fn fxplug_installs_via_zip_pipeline() {
    let Ok(path) = std::env::var("LDOWNLOAD_TEST_FXPLUG") else {
        eprintln!("LDOWNLOAD_TEST_FXPLUG 未设置，跳过");
        return;
    };
    ...
    // 默认校验示例 echo-rewriter；可经 LDOWNLOAD_TEST_FXPLUG_IDENTITY 覆盖以验证其它包。
    let expected = std::env::var("LDOWNLOAD_TEST_FXPLUG_IDENTITY")
        .unwrap_or_else(|_| "ldownload@echo-rewriter".to_string());
