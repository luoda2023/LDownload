...
//!
//! 仅 `plugins` feature 下编译运行。真实执行经 `LDOWNLOAD_TEST_YTDLP=<绝对路径>` 注入。
#![cfg(feature = "plugins")]
...
use ldown_engine::plugin::PluginBridge;
use ldown_engine::plugin::bridge::EngineBridge;
use ldown_engine::plugin::runtime::YtdlpSpec;
...
fn unique_dir(tag: &str) -> PathBuf {
    ...
    d.push(format!("ldownload_yt_{}_{}_{}", tag, std::process::id(), n));
...
async fn make_bridge(data_dir: &Path) -> EngineBridge {
    ...
    // 测试用真实 yt-dlp：`LDOWNLOAD_TEST_YTDLP=<绝对路径>` 时经 config 手动指定，
    // 使 resolve_ytdlp 命中（CI/本机无系统 yt-dlp 时的确定性执行入口）。
    if let Ok(p) = std::env::var("LDOWNLOAD_TEST_YTDLP") {
        db.set_config(ldown_engine::components::CONFIG_YTDLP_PATH, &p)
            .await
...
/// 端到端安装冒烟（需网络，默认忽略）：
/// `cargo test -p ldown_engine --features plugins,components --test plugin_ytdlp -- --ignored ytdlp_install_smoke`
...
async fn ytdlp_install_smoke() {
    ...
    let db = Db::open(&data_dir).await.expect("open db");
    let client = ldown_engine::downloader::build_client(&ProxyConfig::default(), "")
        .expect("build client");
    let progress = |d: u64, t: u64| eprintln!("[install] {d}/{t}");

    let status =
        ldown_engine::components::install_ytdlp(&db, &data_dir, &client, None, &progress)
            .await
    ...
    assert!(
        ldown_engine::components::resolve_ytdlp(&db, &data_dir)
            .await
            .is_some(),
...
/// `cargo test -p ldown_engine --features plugins,components --test plugin_ytdlp -- --ignored ytdlp_list_versions_smoke`
#[cfg(feature = "components")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore]
async fn ytdlp_list_versions_smoke() {
    let client = ldown_engine::downloader::build_client(&ProxyConfig::default(), "")
        .expect("build client");
    let v = ldown_engine::components::list_ytdlp_versions(&client)
        .await
