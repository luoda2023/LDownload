...
//!
//! 用法（绑定本地端口，默认 ignore）：
//!   cargo test -p ldown_engine --test dash_track_pair_progress -- --ignored
...
async fn track_pair_reports_midway_progress_with_real_total() {
    ...
    let work_dir = std::env::temp_dir().join("ldownload-rt-trackpair");
    ...
    ldown_engine::dash_downloader::run_dash_download(params).await;
