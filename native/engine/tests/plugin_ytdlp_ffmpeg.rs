...
//! 转为 mp4。真实执行依赖 ffmpeg，经 `LDOWNLOAD_TEST_FFMPEG=<绝对路径>` 注入；
...
use ldown_engine::plugin::quickjs::QuickJsScriptRuntime;
use ldown_engine::plugin::runtime::{
...
async fn make_bridge(data_dir: &Path, ffmpeg: &str) -> Arc<EngineBridge> {
    ...
    db.set_config(ldown_engine::components::CONFIG_FFMPEG_PATH, ffmpeg)
        .await
...
async fn run_on_done(
    ...
    let script = PluginScript {
        identity: "ldownload@ytdlp".to_string(),
        source: hooks_source(),
...
async fn on_done_converts_webm_to_mp4() {
    let Ok(ffmpeg) = std::env::var("LDOWNLOAD_TEST_FFMPEG") else {
        eprintln!("[skip] 未设置 LDOWNLOAD_TEST_FFMPEG，跳过真实转码");
        return;
    };
...
async fn on_done_skips_when_prefer_mp4() {
    let Ok(ffmpeg) = std::env::var("LDOWNLOAD_TEST_FFMPEG") else {
        eprintln!("[skip] 未设置 LDOWNLOAD_TEST_FFMPEG，跳过门控断言");
        return;
    };
