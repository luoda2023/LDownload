...
//!
//! 用法（绑定本地端口，默认 ignore）：
//!   cargo nextest run -p ldown_engine --test dash_track_pair_resume --run-ignored all
...
use ldown_engine::downloader::{DownloadParams, ProgressUpdate, RequestSpec, build_client};
...
fn make_params(
    ...
    DownloadParams {
        ...
        range_verified: true,
        db: db.clone(),
        client: build_client(&ProxyConfig::default(), "LDownloadRealTest/1.0").expect("client"),
        progress_tx: tx,
        cancel_token: cancel,
        ...
        selector: Arc::new(ldown_engine::NoopSelection),
        checksum: String::new(),
...
async fn track_pair_multi_segment_pause_resume_and_pair_coordinates() {
    ...
    let work_dir = std::env::temp_dir().join("ldownload-rt-trackpair-resume");
    ...
    ldown_engine::dash_downloader::run_dash_download(make_params(
    ...
    let temp_path = work_dir.join(format!("pair.mp4{}", ldown_engine::downloader::TEMP_EXT));
    assert!(
