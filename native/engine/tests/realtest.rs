...
//! LDownload 引擎的真实代码路径（`run_coordinated_download` / `download_single` /
...
use ldown_engine::downloader::{ProgressUpdate, RequestSpec, build_client, resolve_file_info};
use ldown_engine::events::{EngineEvent, EventSink};
use ldown_engine::proxy_config::ProxyConfig;
use ldown_engine::segment_coordinator::run_coordinated_download;
use ldown_engine::speed_limiter::SpeedLimiter;
...
async fn handle_conn(mut stream: TcpStream, st: Arc<ServerState>) -> std::io::Result<()> {
    ...
    // 全量当前文件**。LDownload 的续传/分段修复正是依赖这一点来检出"文件中途变化"。
...
fn test_client() -> reqwest::Client {
    ...
    build_client(&proxy, "LDownloadRealTest/1.0").expect("build_client")
}

fn unique_dir(tag: &str) -> std::path::PathBuf {
    let d = std::env::temp_dir().join(format!("ldownload_realtest_{}_{}", tag, std::process::id()));
...
async fn run_coord(
    ...
    Result<(), ldown_engine::downloader::DownloadError>,
    std::path::PathBuf,
) {
    ...
    let res = run_coordinated_download(
        ...
        false,
        segments,
        ldown_engine::cdn::NodePool::single(client.clone()),
        &db,
        &tx,
        cancel,
        &speed_limiter,
        ...
        etag,
        "",
        ldown_engine::segment_coordinator::ReportScope::whole_task(),
        0,
        false,
        None,
    )
...
async fn run_full(
    ...
    use ldown_engine::downloader::{DownloadParams, run_download};
    ...
    let params = DownloadParams {
        ...
        selector: std::sync::Arc::new(ldown_engine::NoopSelection),
        ...
        ffmpeg_path: None,
        cdn: ldown_engine::cdn::CdnTaskInput::default(),
    };
...
async fn use_server_time_uses_new_last_modified_after_version_change() {
    use ldown_engine::downloader::{DownloadParams, TEMP_EXT, run_download};
...
async fn hint_no_content_length_truncation_single_stream_must_not_be_accepted() {
    ...
    // 失败任务绝不能产出最终成品文件（.ldownload 临时文件允许残留）。
    assert!(
        !dest.exists(),
...
/// ```text
/// LDOWNLOAD_RT_URL="http://nas:1080/multiple-download?token=..." \
/// LDOWNLOAD_RT_SIZE=32583874 \
/// LDOWNLOAD_RT_COOKIES="fnos-token=..." \
/// cargo test -p ldown_engine --test realtest manual_real_url -- --ignored --nocapture
/// ```
#[tokio::test(flavor = "current_thread")]
#[ignore = "manual: requires LDOWNLOAD_RT_URL pointing at a live endpoint"]
async fn manual_real_url_hint_download() {
    use ldown_engine::downloader::{DownloadParams, run_download};

    let Ok(url) = std::env::var("LDOWNLOAD_RT_URL") else {
        eprintln!("LDOWNLOAD_RT_URL 未设置 — 跳过");
        return;
    };
    let size: i64 = std::env::var("LDOWNLOAD_RT_SIZE")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(-1); // -1 = 大小未知但确认可下载（同扩展 webRequest 嗅探语义）
    let cookies = std::env::var("LDOWNLOAD_RT_COOKIES").unwrap_or_default();
    ...
    // UA 与桌面 App 一致：LDOWNLOAD_RT_UA 显式覆盖，空 = build_client 内置
    // Chrome UA（fnOS 等端点对非浏览器 UA 直接 400，test_client 的
    // "LDownloadRealTest/1.0" 会被拒）。
    let ua = std::env::var("LDOWNLOAD_RT_UA").unwrap_or_default();
