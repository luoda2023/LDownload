...
const FILE_BODY: &[u8] = b"ldownload plugin lazy resolve integration payload body!!\n";
...
async fn create(engine: &mut Engine, url: &str, save_dir: &str, name: &str) -> String {
    engine
        .manager
        .create_task(ldown_engine::download_manager::NewTaskSpec {
            url: url.to_string(),
...
async fn lazy_resolve_rewrites_and_downloads() {
    let work = std::env::temp_dir().join(format!("ldownload-plugin-it-{}", uuid_like()));
...
async fn disabled_plugin_passes_through() {
    let work = std::env::temp_dir().join(format!("ldownload-plugin-dis-{}", uuid_like()));
    tokio::fs::create_dir_all(&work).await.expect("mkdir work");
    let (port, _srv) = spawn_server();
    let real_url = format!("http://127.0.0.1:{port}/real.bin");
...
async fn resume_of_errored_resolver_task_reresolves() {
    let work = std::env::temp_dir().join(format!("ldownload-plugin-resume-{}", uuid_like()));
    tokio::fs::create_dir_all(&work).await.expect("mkdir work");
    let (port, _srv) = spawn_server();
    let real_url = format!("http://127.0.0.1:{port}/real.bin");
...
async fn pause_resume_during_resolve_window_dispatches_resume() {
    let work = std::env::temp_dir().join(format!("ldownload-plugin-race-{}", uuid_like()));
    tokio::fs::create_dir_all(&work).await.expect("mkdir work");
    let (port, _srv) = spawn_server();
    let real_url = format!("http://127.0.0.1:{port}/real.bin");
...
async fn resume_applies_fresh_resolver_extra_headers() {
    let work = std::env::temp_dir().join(format!("ldownload-plugin-hdr-{}", uuid_like()));
    tokio::fs::create_dir_all(&work).await.expect("mkdir work");
...
async fn uninstall_clears_task_resolver_binding() {
    let work = std::env::temp_dir().join(format!("ldownload-plugin-uninst-{}", uuid_like()));
    tokio::fs::create_dir_all(&work).await.expect("mkdir work");
    let (port, _srv) = spawn_server();
    let real_url = format!("http://127.0.0.1:{port}/real.bin");
...
async fn resume_with_disabled_plugin_fails_closed() {
    let work = std::env::temp_dir().join(format!("ldownload-plugin-failclosed-{}", uuid_like()));
    tokio::fs::create_dir_all(&work).await.expect("mkdir work");
    let (port, _srv) = spawn_server();
    let real_url = format!("http://127.0.0.1:{port}/real.bin");
