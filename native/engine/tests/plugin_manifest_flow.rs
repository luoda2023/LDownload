...
use ldown_engine::download_manager::{CreateGroupSpec, GroupItemSpec, NewTaskSpec};
...
const FILE_BODY: &[u8] = b"ldownload plugin manifest flow integration payload!!\n";
...
async fn fission_end_to_end_creates_group_and_downloads_members() {
    let work = std::env::temp_dir().join(format!("ldownload-manifest-it-{}", uuid_like()));
