...
use ldown_engine::downloader::{ProgressUpdate, build_client, resolve_file_info};
...
/// 用 LDownload 自己的 build_client 单流下载，作为 baseline。
...
/// 单次完整的 multi-segment 下载，使用 LDownload 真实代码路径。
///
/// 返回 (实际文件大小, SHA256, 耗时)。
async fn run_one_real_download(
    ...
    // 构造 LDownload 真实 client（与生产代码完全一致）
    let proxy = ProxyConfig::default();
    let client = build_client(&proxy, "Mozilla/5.0 LDownloadCorruptionTest/1.0")
    ...
    let spec = ldown_engine::downloader::RequestSpec::empty_get();
...
async fn real_multi_segment_corruption_repeat() {
    ...
    println!("==========================================");
    println!("LDownload 真实代码 multi-segment 损坏测试");
    ...
    // ---- Step 0: probe（与 LDownload 生产代码一致）----
    println!("\n[Step 0] resolve_file_info — 模拟 LDownload probe");
    let proxy = ProxyConfig::default();
    let client = build_client(&proxy, "Mozilla/5.0 LDownloadCorruptionTest/1.0")
    ...
    let info = resolve_file_info(
        &client,
        TEST_URL,
        &ldown_engine::downloader::RequestSpec::empty_get(),
    )
    .await
    ...
    let (base_size, base_sha) = download_baseline(
        TEST_URL,
        &base_dest,
        "Mozilla/5.0 LDownloadCorruptionTest/1.0",
    )
    .await
    ...
    if corrupt_count == 0 && error_count == 0 {
        ...
        println!("   说明 LDownload segment_coordinator 在该场景下行为正确");
    } else {
        println!(
            "\n❌ 复现了问题：{} 次损坏 / {} 次错误（共 {} 次）",
