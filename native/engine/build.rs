//! 构建脚本：从仓库根 pubspec.yaml 读取应用版本号，注入 `LDOWNLOAD_APP_VERSION`
//! 编译期环境变量，供 downloader.rs 拼出 aria2 风格的默认 UA（`LDownload/<版本>`）。
//!
...
fn main() {
    ...
    // pubspec 版本形如 `version: 10.0.3+1`，取 `+` 前的语义版本。
    ...
    println!("cargo:rustc-env=LDOWNLOAD_APP_VERSION={version}");
}
