//! CLI 式同进程直接调用 `ldown_engine` 的最小可执行证明。
//!
//! 不依赖 `hub`/`rinf` 的任何符号,仅 `use ldown_engine::*`,构造
//! `Engine::new(config, Arc::new(NoopSink), Arc::new(NoopSelection))`,对一个
//! 本地 HTTP 服务器提供的小文件发起一次完整的"创建任务 → 下载 → 完成"流程。
//!
//! 运行:
//! ```text
//! cargo run --example headless_download -p ldown_engine
//! ```
//!
...
/// 固定内容,便于校验下载结果字节完全一致。
const FILE_BODY: &[u8] = b"ldownload-engine headless download example payload\n";
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let work_dir = std::env::temp_dir().join(format!(
        "ldownload-engine-headless-example-{}",
        std::process::id()
    ));
