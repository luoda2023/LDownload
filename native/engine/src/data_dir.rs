...
//!
//! Determines where LDownload stores persistent data (database, logs, NMH manifests).
//!
//! ## Strategy
//!
...
//! | Android         | —          | `/data/data/<package>/files/ldownload/`           |
//!
//! ### 便携模式检测（仅 Windows）
//!
...
//! - SQLite 三件套（`ldownload.db` / `-wal` / `-shm`）作为原子组迁移，
...
///
/// ```
/// use ldown_engine::data_dir::resolve_data_dir;
///
...
fn resolve_data_dir_inner() -> PathBuf {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Installed mode: use %LOCALAPPDATA%\LDownload (always user-writable).
        if let Some(local) = std::env::var_os("LOCALAPPDATA") {
            return PathBuf::from(local).join("LDownload");
        }
        // Fallback: %APPDATA%\LDownload
        if let Some(appdata) = std::env::var_os("APPDATA") {
            return PathBuf::from(appdata).join("LDownload");
        }
        // Last resort: exe directory (may fail on write, but better than ".").
        exe_dir()
    }

    // Android: 应用内部存储 `/data/data/<package>/files/ldownload`。
    // 包名 = 进程名（`/proc/self/cmdline` 首个 NUL 之前的内容）。
    // 该目录无需任何存储权限即可读写，与 Dart 侧 `resolveDataDir()` 保持一致。
    #[cfg(target_os = "android")]
    {
        match android_package_name() {
            Some(pkg) => PathBuf::from(format!("/data/data/{pkg}/files/ldownload")),
            None => exe_dir(),
        }
    }
...
const DB_FILE: &str = "ldownload.db";
#[cfg(any(target_os = "windows", test))]
const DB_WAL: &str = "ldownload.db-wal";
#[cfg(any(target_os = "windows", test))]
const DB_SHM: &str = "ldownload.db-shm";
...
mod tests {
    ...
    fn fresh_root(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "ldownload_portable_migrate_{name}_{}",
            std::process::id()
        ));
    ...
    fn migrates_known_items_and_db_group() {
        ...
        write(&root.join("logs").join("ldownload_2026-01-01.log"), "log");
        // exe 根层的非数据文件（exe/DLL）不在清单内，必须原地保留。
        write(&root.join("ldownload.exe"), "bin");
        ...
        assert!(
            new_dir
                .join("logs")
                .join("ldownload_2026-01-01.log")
                .exists()
        );
        assert!(!root.join(DB_FILE).exists());
        assert!(!root.join("icons").exists());
        assert!(root.join("ldownload.exe").exists());
