...
impl Db {
    /// 在 `dir` 目录下打开（不存在则创建）SQLite 数据库 `ldownload.db`。
    ///
    ...
    pub async fn open(dir: &Path) -> Result<Self, DbError> {
        ...
        let db_path = dir.join("ldownload.db");
        let url = format!(
            "sqlite:{}?mode=rwc",
    ...
    ///
    /// ```no_run
    /// # async fn run() -> Result<(), ldown_engine::db::DbError> {
...
mod tests {
    ...
    //
    // Run with:  cargo test -p ldown_engine -- --nocapture delete_benchmark
    // -----------------------------------------------------------------------
    ...
    async fn reopen_same_dir_is_idempotent() {
        let n = TEST_COUNTER.fetch_add(1, Ordering::Relaxed);
        let dir =
            std::env::temp_dir().join(format!("ldownload_reopen_{}_{}", std::process::id(), n));
        std::fs::create_dir_all(&dir).expect("create temp dir");
        {
    ...
    ///  cargo test -p ldown_engine -- --ignored pg_smoke`
    #[tokio::test]
    ...
    // -----------------------------------------------------------------------
    // 文件跟踪（LDownload #11）：update_task_file_missing / file_missing 读回一致性
    // -----------------------------------------------------------------------
