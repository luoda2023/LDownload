...
///
/// ```
/// use ldown_engine::plugin::manifest::url_glob_match;
///
...
mod tests {
    ...
    #[test]
    fn identity_rules() {
        assert!(is_valid_identity("ldownload@youtube"));
        assert!(is_valid_identity("a_b-c@x-y_z"));
        assert!(!is_valid_identity("dicad.cn@youtube")); // 禁 '.'
        assert!(!is_valid_identity("Ldownload@youtube")); // 禁大写
        assert!(!is_valid_identity("noat"));
        assert!(!is_valid_identity("@youtube"));
        assert!(!is_valid_identity("ldownload@"));
