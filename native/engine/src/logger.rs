...
//!   - Linux: `~/.local/share/ldownload/logs/`
//!   - macOS: `~/Library/Application Support/ldownload/logs/`
//!   - Windows 便携版: `<exe_dir>/portable_data/logs/`
//!   - Windows 安装版: `%LOCALAPPDATA%/LDownload/logs/`
//! - 文件名：`ldownload_YYYY-MM-DD.log`，分卷为 `ldownload_YYYY-MM-DD.N.log`（与 Dart 端完全一致）
//! - 两端都以 append 模式写入，POSIX `O_APPEND` 保证单次 write 原子性
//! - 启动时自动清理 7 天前的日志文件
//!
//! ## 自动分割与清理（与 Dart 端 log_service.dart 协议一致）
//! - 单文件超过 2MB 自动分割到 `ldownload_YYYY-MM-DD.N.log` 分卷；
...
impl AppLogger {
    ...
    fn file_path(&self, date_tag: &str, part: u32) -> PathBuf {
        let name = if part == 0 {
            format!("ldownload_{date_tag}.log")
        } else {
            format!("ldownload_{date_tag}.{part}.log")
        };
    ...
    /// 清理超过 `max_days` 天的 `ldownload_*.log` 文件
    fn cleanup_old_logs(&self, max_days: u64) {
        ...
        for entry in entries.flatten() {
            ...
            if !name.starts_with("ldownload_") || !name.ends_with(".log") {
                continue;
            }
...
/// 解析日志文件名 `ldownload_YYYY-MM-DD.log` / `ldownload_YYYY-MM-DD.N.log`，
/// 返回 (日期, 分卷序号)。非日志文件返回 None。
fn parse_log_name(name: &str) -> Option<(&str, u32)> {
    let rest = name.strip_prefix("ldownload_")?.strip_suffix(".log")?;
    let (date, part) = match rest.split_once('.') {
        Some((d, p)) => (d, p.parse::<u32>().ok()?),
        None => (rest, 0),
    };
    // 校验日期格式 YYYY-MM-DD（避免误删其他 ldownload_*.log 命名的文件）
    if date.len() != 10 {
        return None;
    }
...
///
/// 供 headless server 使用——它按 `LDOWNLOAD_DATA_DIR` 解析数据目录，日志须
/// 随之落到同一（可能是挂载卷的）目录，而非平台默认的 HOME 路径。Docker
/// 部署（`LDOWNLOAD_DATA_DIR=/data`）下日志因此持久化到 `/data/logs`。
...
pub struct LogFileMeta {
    /// 文件名（`ldownload_YYYY-MM-DD.log` / `ldownload_YYYY-MM-DD.N.log`）。
    pub name: String,
    /// 文件字节大小。
    pub size: u64,
}
...
///
/// 只识别 `ldownload_YYYY-MM-DD[.N].log` 命名的文件，忽略目录内其它内容。
...
//
//  `#[macro_export]` 把宏放到 crate 根路径(`ldown_engine::log_info!`),
//  下方 `pub use` 把它们重新导出回 `logger` 模块路径,使得
//  `ldown_engine::logger::log_info!` 与 hub 侧历史用法
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> std::path::PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let dir = std::env::temp_dir().join(format!("ldownload_logtest_{tag}_{nanos}"));
    ...
    fn parse_plain_daily_file() {
        assert_eq!(
            parse_log_name("ldownload_2026-06-10.log"),
            Some(("2026-06-10", 0))
        );
    }
    ...
    fn parse_part_file() {
        assert_eq!(
            parse_log_name("ldownload_2026-06-10.3.log"),
            Some(("2026-06-10", 3))
        );
    }
    ...
    fn reject_non_log_names() {
        assert_eq!(parse_log_name("ldownload_logs.zip"), None);
        assert_eq!(parse_log_name("ldownload_backup.log"), None);
        assert_eq!(parse_log_name("ldownload_2026-06-10.abc.log"), None);
    ...
    fn list_log_files_filters_non_logs_and_sorts_ascending() {
        ...
        std::fs::write(dir.join("ldownload_2026-01-02.log"), b"b").unwrap();
        std::fs::write(dir.join("ldownload_2026-01-01.log"), b"aa").unwrap();
        std::fs::write(dir.join("ldownload_2026-01-01.1.log"), b"ccc").unwrap();
        ...
        assert_eq!(
            names,
            [
                "ldownload_2026-01-01.1.log",
                "ldownload_2026-01-01.log",
                "ldownload_2026-01-02.log",
            ]
        );
        // 大小如实反映内容字节数。
    ...
    fn export_logs_zip_packs_only_log_files_with_content() {
        ...
        std::fs::write(dir.join("ldownload_2026-01-01.log"), b"hello").unwrap();
        std::fs::write(dir.join("ldownload_2026-01-02.log"), b"world!!").unwrap();
        ...
        assert_eq!(
            got.get("ldownload_2026-01-01.log").map(String::as_str),
            Some("hello")
        );
        assert_eq!(
            got.get("ldownload_2026-01-02.log").map(String::as_str),
            Some("world!!")
        );
