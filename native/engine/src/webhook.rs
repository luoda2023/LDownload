...
impl WebhookEventKind {
    ...
    /// wire 名（payload `event` 字段与 `X-LDownload-Event` 头）。
...
/// 事件里的任务快照。字段名对齐 `ldown_api::types::TaskDto`，不另起一套。
    ...
    fn default_template(self) -> &'static str {
        match self {
            ...
            Preset::Ntfy => concat!(
                ...
                r#""message":"{event.summary}","tags":["ldownload"]}"#
            ),
            Preset::Gotify => {
                r#"{"title":"{event.title}","message":"{event.summary}","priority":5}"#
            }
            Preset::Bark => {
                r#"{"title":"{event.title}","body":"{event.summary}","group":"LDownload"}"#
            }
...
impl WebhookDispatcher {
    ...
    pub fn new(proxy_config: &ProxyConfig) -> Self {
        let inner = Inner {
            ...
            instance: InstanceInfo {
                app: "ldownload",
                version: env!("LDOWNLOAD_APP_VERSION"),
                host: detect_host(),
            },
...
fn build_clients(proxy_config: &ProxyConfig) -> Clients {
    // UA 传空 → downloader 用内置 `LDownload/<version>`（设计要求的固定 UA）。
...
impl Inner {
    ...
    async fn deliver(
        ...
        let mut headers: Vec<(String, String)> = vec![
            ...
            (
                "X-LDownload-Event".to_string(),
                event.kind.wire().to_string(),
            ),
            ("X-LDownload-Delivery".to_string(), delivery_id.clone()),
        ];
        if !spec.sign_secret.is_empty() {
            let sig = sign_header(&spec.sign_secret, now.timestamp(), &body);
            if !sig.is_empty() {
                headers.push(("X-LDownload-Signature".to_string(), sig));
            }
        }
...
mod tests {
    ...
    /// 记录收到的请求数与 `X-LDownload-Event` 头到达顺序，供重试/保序断言。
    struct MockServer {
    ...
    fn spawn_mock(status_line: &'static str) -> MockServer {
        ...
        std::thread::spawn(move || {
            for stream in listener.incoming() {
                ...
                if let Some(rest) = text.split("x-ldownload-event:").nth(1)
