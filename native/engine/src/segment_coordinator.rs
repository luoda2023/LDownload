...
/// 进度报告/DB 持久化）。尾部微拆分在 LDownload 架构下以极小改动覆盖了 90%+
...
mod tests {
    ...
    async fn conn_caps_persist_and_reload_roundtrip() {
        ...
        let dir = std::env::temp_dir().join(format!("ldownload_connp_{}", uuid::Uuid::new_v4()));
    ...
    async fn open_sync_gate_test_file() -> (std::path::PathBuf, tokio::fs::File) {
        ...
        std::fs::write(&path, b"ldownload-file-sync-gate-test").expect("write temp file content");
