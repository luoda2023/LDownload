...
impl DownloadManager {
    ...
    /// # async fn run() -> Result<(), ldown_engine::EngineError> {
    /// # let config = EngineConfig { max_concurrent: 5, speed_limit_bps: 0, upload_limit_bps: 0, default_save_dir: "/tmp/downloads".to_string(), app_data_dir: "/tmp/ldownload".to_string(), bt_config: BtConfig::default(), proxy_config: ProxyConfig::default(), user_agent: String::new(), data_dir_override: None, database_url: None };
...
mod tests {
    ...
    // -------------------------------------------------------------------------
    // 文件跟踪（LDownload #11）：task_target_path / probe_missing / scan_missing_files
    // -------------------------------------------------------------------------

    /// LDownload #11：空名与路径穿越/绝对路径必须解析为 `None`——无法安全判定
    ...
    fn unique_filetrack_test_dir(tag: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "ldownload_filetrack_test_{tag}_{}_{}",
            std::process::id(),
    ...
    /// LDownload #11 核心契约：completed 任务的目标文件消失后 `file_missing`
