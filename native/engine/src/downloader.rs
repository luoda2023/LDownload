...
// ---------------------------------------------------------------------------
//
// 设计动机：LDownload 早期把每个下载视为"URL → 内容"的简化模型，所有
...
///
/// ```
/// use ldown_engine::downloader::is_valid_referrer;
///
...
/// `LDownload/<pubspec 版本号>`（build.rs 注入 `LDOWNLOAD_APP_VERSION`），
/// debug 构建固定 `LDownload/1.0`。
const DEFAULT_UA: &str = if cfg!(debug_assertions) {
    "LDownload/1.0"
} else {
    concat!("LDownload/", env!("LDOWNLOAD_APP_VERSION"))
};
...
pub async fn resolve_file_info(
    ...
    for attempt in 0..PROBE_MAX_RETRIES {
        ...
        // the request falls back to DEFAULT_UA ("LDownload/<version>").  This
    ...
    if effective_total_bytes > 0 && !decompressed_single {
        if actual_use_segments {
            ...
            if file_len != effective_total_bytes {
                ...
                // LDownload's own request (dynamic tokens, re-generated PDFs,
...
mod tests {
    ...
    async fn dedup_filename_no_conflict() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_no_conflict");
    ...
    async fn dedup_filename_with_conflict() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_conflict");
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_case_folds_across_disk_variants() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_case_fold");
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_temp_file_conflict() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_temp");
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_no_extension() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_noext");
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_reserved_set_avoids_collision() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_reserved");
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_reserved_set_phase2_collision() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_reserved_p2");
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    //         都被加 (1)（用户日志：LDownload-0.1.40-windows-x64-setup.exe →
    //         LDownload-0.1.40-windows-x64-setup (1).exe，磁盘并无原名文件）。
    //
    ...
    async fn dedup_filename_async_no_self_conflict_when_alone() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_no_self_conflict_async");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_overwrite_keeps_name_when_only_final_exists() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_ow_final");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_overwrite_temp_file_still_conflicts() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_ow_temp");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_overwrite_reserved_and_avoid_still_conflict() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_ow_reserved");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn dedup_filename_overwrite_directory_still_conflicts() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_ow_dir");
        let _ = tokio::fs::remove_dir_all(&dir).await;
    ...
    async fn claim_rename_succeeds_when_dst_free() {
        let dir = std::env::temp_dir().join("ldownload_test_claim_rename_free");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        let _ = tokio::fs::create_dir_all(&dir).await;
    ...
    async fn claim_rename_fails_when_dst_exists_preserves_both() {
        let dir = std::env::temp_dir().join("ldownload_test_claim_rename_exists");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        let _ = tokio::fs::create_dir_all(&dir).await;
        let src = dir.join("src.bin");
        let dst = dir.join("dst.bin");
    ...
    async fn dedup_filename_avoid_param_case_folds_and_renames() {
        let dir = std::env::temp_dir().join("ldownload_test_dedup_avoid_case_fold");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        let _ = tokio::fs::create_dir_all(&dir).await;
