...
///
/// ```
/// let avail = ldown_engine::disk_space::available_space(&std::env::temp_dir());
...
fn available_space_impl(dir: &Path) -> Option<u64> {
    ...
    // 相同的大卷截断风险——LDownload 仅支持 64 位桌面端,不在支持矩阵内。
...
mod tests {
    ...
    fn available_space_returns_none_for_missing_path() {
        let missing = std::env::temp_dir().join("ldownload_definitely_missing_dir_xyz");
