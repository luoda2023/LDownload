...
///
/// LDownload manages task state in SQLite and re-adds torrents itself on
...
impl SharedBtSession {
    ...
    pub fn new(
        ...
        // in the app data directory (next to ldownload.db), NOT in the user's
...
///   `save_dir/<torrent name>/...` layout even though LDownload downloads into
...
fn compute_completion_layout(input: CompletionLayoutInput<'_>) -> Option<CompletionLayout> {
    ...
    // torrents under `<torrent name>/`. LDownload overrides rqbit's
...
mod tests {
    ...
    fn clear_stale_session_state_tolerates_missing_files() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "ldownload_bt_session_test_missing_{}",
            std::process::id()
        ));
        let _ = std::fs::create_dir_all(&dir);
        super::clear_stale_session_state(&dir);
    ...
    fn unique_test_dir(tag: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "ldownload_bt_stage_test_{tag}_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map(|d| d.as_nanos())
