//! 持久化 CLI 配置 —— `ldownload config set/unset/get/list` 写入的本地配置文件，
...
//!
//! 1. 命令行 flag（`--token X`）
//! 2. 环境变量（`LDOWNLOAD_TOKEN` —— 由 clap 的 `env=` 合并进 flag）
...
//! | 平台 | 路径 |
//! |---|---|
//! | Windows | `%APPDATA%\LDownload\config\cli.toml` |
//! | Linux | `$XDG_CONFIG_HOME/ldownload/cli.toml`（默认 `~/.config/ldownload/`） |
//! | macOS | `~/Library/Application Support/dev.ldownload/cli.toml` |
//!
//! ## 安全
//!
...
pub struct CliConfig {
    /// 服务基址（对应 `--url` / `LDOWNLOAD_URL`）。
    #[serde(skip_serializing_if = "Option::is_none")]
    pub url: Option<String>,
    /// 管理 API token（对应 `--token` / `LDOWNLOAD_TOKEN`）。
    #[serde(skip_serializing_if = "Option::is_none")]
...
///
/// ```no_run
/// use ldown_cli::config::config_path;
///
/// let p = config_path().expect("a config dir should exist");
/// assert!(p.ends_with("cli.toml"));
/// ```
pub fn config_path() -> Result<PathBuf, ConfigError> {
    let dirs = ProjectDirs::from("dev", "ldownload", "app").ok_or(ConfigError::NoConfigDir)?;
