//! Typed HTTP 客户端 —— 复用 [`ldown_api`] 的路径常量与 wire 类型，
//! 保证请求地址/JSON 结构与服务端永不漂移。
//!
//! 面向运行中的 LDownload App（本机 API 服务，默认 `127.0.0.1:17800`）或
//! headless server。所有管理 API 强制 token 鉴权。

use std::time::Duration;

use ldown_api::auth::TOKEN_HEADER;
use ldown_api::routes;
use ldown_api::service::UNKNOWN_ENDPOINT_MESSAGE;
use ldown_api::types::{
...
impl ClientError {
    ...
    ///
    /// ```
    /// use ldown_cli::client::ClientError;
...
impl ApiClient {
    ...
    /// - `token`：管理 API token（`LDOWNLOAD_TOKEN`）。
    /// - `timeout`：单请求超时。
    ...
    fn transport_err(&self, e: &reqwest::Error) -> ClientError {
        if e.is_timeout() {
            ...
            ClientError::new(
                format!("cannot connect to {} — is LDownload running?", self.base),
                ExitCode::Network,
            )
        } else {
            ...
            return ClientError::new(
                format!(
                    "management API is not enabled on {} — turn on \
                     ...
                     or run a headless server (where it is always on), then set LDOWNLOAD_TOKEN",
                    self.base
                ),
                ExitCode::NotFound,
            );
