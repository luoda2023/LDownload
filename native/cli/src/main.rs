//! LDownload CLI 入口 —— aria2c 风格命令行下载客户端。

use std::process::ExitCode as ProcExitCode;
use std::time::Duration;

use clap::{Args, Parser, Subcommand};
use ldown_api::types::{CreateTaskRequest, RssSourceDto};
use ldown_cli::client::{ApiClient, ClientError};
use ldown_cli::config::{CliConfig, ConfigError};
use ldown_cli::exit::ExitCode;
use ldown_cli::format::{
...
/// LDownload 命令行下载客户端。
#[derive(Debug, Parser)]
#[command(name = "ldownload", version, about, long_about = None)]
struct Cli {
    /// 服务基址（默认 http://127.0.0.1:17800）。
    #[arg(long, global = true, env = "LDOWNLOAD_URL")]
    url: Option<String>,

    /// 管理 API token。
    #[arg(long, global = true, env = "LDOWNLOAD_TOKEN")]
    token: Option<String>,
...
fn main() -> ProcExitCode {
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("ldownload: failed to start runtime: {e}");
...
fn cmd_config(action: ConfigCmd, json: bool) -> Result<(), ConfigError> {
    match action {
        ...
        ConfigCmd::Path => {
            println!("{}", ldown_cli::config::config_path()?.display());
            Ok(())
        }
    }
}
...
async fn cmd_ping(client: &ApiClient, json: bool) -> Result<(), ClientError> {
    ...
    if json {
        ...
        let app = v.get("app").and_then(|x| x.as_str()).unwrap_or("LDownload");
...
async fn cmd_add(client: &ApiClient, a: AddArgs, json: bool) -> Result<(), ClientError> {
    ...
    for url in urls {
        ...
        match client.create_task(&req).await {
            ...
            Err(e) => {
                eprintln!("ldownload: failed to add {url}: {e}");
