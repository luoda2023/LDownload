//! B 模式：内嵌 [`ldown_engine::Engine`] 的一次性独立下载（`ldownload add --local`）。
//!
...
use ldown_engine::download_manager::{NewTaskSpec, progress_reporter};
...
pub async fn run_add_local(args: AddArgs, json: bool) -> Result<(), ClientError> {
    ...
    for url in &urls {
        ...
        match id {
            ...
            None => {
                eprintln!("ldownload: failed to create task for {url}");
