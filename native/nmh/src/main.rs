...
//!   - Diagnostic log is written to `%TEMP%/ldown_nmh.log`.
...
/// IPC path for communicating with the LDownload desktop app.
/// Windows uses a Named Pipe; Linux/macOS uses a Unix Domain Socket.
#[cfg(windows)]
const PIPE_NAME: &str = r"\\.\pipe\ldownload";

/// LDownload App executable name (Windows only).
#[cfg(windows)]
const APP_EXE_NAME: &str = "ldownload.exe";
...
// ---------------------------------------------------------------------------
// Diagnostic logging (writes to %TEMP%/ldown_nmh.log)
// ---------------------------------------------------------------------------
...
fn log_path() -> Option<std::path::PathBuf> {
    #[cfg(windows)]
    {
        std::env::var("TEMP")
            ...
            .ok()
            .map(|tmp| Path::new(&tmp).join("ldown_nmh.log"))
    }
    #[cfg(target_os = "macos")]
    {
        ...
        if let Some(home) = home_dir() {
            ...
            return Some(dir.join("ldown_nmh.log"));
        }
        Some(Path::new("/tmp").join("ldown_nmh.log"))
    }
    #[cfg(all(not(windows), not(target_os = "macos")))]
    {
        // Linux: use ~/.local/share/ldownload/ldown_nmh.log
        ...
        if let Some(home) = home_dir() {
            let dir = home.join(".local").join("share").join("ldownload");
            let _ = std::fs::create_dir_all(&dir);
            return Some(dir.join("ldown_nmh.log"));
        }
        Some(Path::new("/tmp").join("ldown_nmh.log"))
...
// Non-Windows: connect to LDownload via Unix Domain Socket.
#[cfg(not(windows))]
mod pipe {
    use std::io::{self, Read, Write};
    use std::os::unix::net::UnixStream;

    /// Resolve the Unix socket path that the LDownload app is listening on.
    /// Must match the path used in native/hub/src/native_messaging.rs.
    fn socket_path() -> std::path::PathBuf {
        #[cfg(target_os = "macos")]
        {
            // macOS: ~/Library/Application Support/ldownload/ldownload.sock
            ...
            if let Some(home) = super::home_dir() {
                let dir = home
                    ...
                    .join("ldownload");
                let _ = std::fs::create_dir_all(&dir);
                return dir.join("ldownload.sock");
            }
        }
        // Linux: use ~/.local/share/ldownload/ldownload.sock
        ...
        if let Ok(dir) = std::env::var("XDG_RUNTIME_DIR") {
            std::path::Path::new(&dir).join("ldownload.sock")
        } else {
            std::path::Path::new("/tmp").join("ldownload.sock")
        }
    }
    ...
    impl PipeHandle {
        /// Connect to the LDownload Unix socket. Returns None if the app is not running.
...
/// Find the LDownload App executable.
///
/// Search order:
...
fn find_app_exe() -> Option<PathBuf> {
    ...
    // flutter run / flutter build macos uses the display name ("LDownload"),
    // while production archives may differ. Search both variants.
    const APP_EXE_CANDIDATES: &[&str] = &["LDownload", "ldownload"];
    const APP_BUNDLE_CANDIDATES: &[&str] = &["LDownload.app", "ldownload.app"];
...
fn find_app_exe() -> Option<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let candidate = dir.join("ldownload");
        if candidate.exists() {
            return Some(candidate);
        }
    }
...
/// Launch the LDownload App as a detached process.
#[cfg(windows)]
