...
//! 支持 aria2 约定的 `token:xxx`（params[0]）或 `X-LDownload-Token` 头鉴权。
...
///
/// `header_token_ok`：`X-LDownload-Token` 头是否已通过校验（由 HTTP 层判定）；
...
/// 接受 `X-LDownload-Token` 头（已由 HTTP 层判定）或 aria2 约定的
/// `params[0] = "token:xxx"`。
...
/// `aria2.remove`/`aria2.forceRemove`：LDownload 不区分「优雅/强制」停止，
...
/// `aria2.pause`/`aria2.forcePause`：LDownload 不区分「优雅/强制」暂停。
