...
//!
//! 让 AI 客户端（Claude Desktop / Cursor / Cline 等）把 LDownload 当作 MCP
...
//! （`Authorization: Bearer <token>` 或 `X-LDownload-Token`）。规范允许内部
...
fn initialize_result(params: &Value, app_version: &str) -> Value {
    ...
    json!({
        ...
        "serverInfo": { "name": "LDownload", "version": app_version },
    })
}
...
mod tests {
    ...
    async fn initialize_echoes_version_and_declares_tools() {
        ...
        assert_eq!(result["serverInfo"]["name"], "LDownload");
