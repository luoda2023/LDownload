...
//! `ldown_api`（axum 0.8 + [`ApiHost`] 抽象）的端到端验证：不再手写 HTTP 解析，
...
#[tokio::test]
async fn management_api_accepts_bearer_or_x_ldownload_token_header() {
...
/// 且 `Allow-Headers` 原样回显请求头清单——否则自定义头（`X-LDownload-Client`
