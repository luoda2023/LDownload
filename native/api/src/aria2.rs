...
/// LDownload `task_id`（UUID v4 字符串）→ aria2 兼容 GID：
...
/// LDownload `TaskDto.status` → aria2 `status` 字符串。
///
...
/// LDownload 引擎不是按 piece 调度、不做逐连接计数、HTTP 任务不统计上传
...
/// 单文件条目（`getFiles`/`tellStatus.files`）：LDownload 任务恒单文件，
...
/// `getUris`/`files[].uris`：LDownload 只跟踪单一 URL，非空时返回一个
...
/// 一条 aria2 全局选项 ↔ LDownload config key 的映射规则。
...
/// 契约表：`local://aria2_compat_contract.md` §「aria2 全局选项 ↔ LDownload
/// config key 映射」。
...
/// AriaNg 等客户端常探测、但 LDownload 无对应可写配置的选项：给出 aria2
...
/// LDownload 不支持的 `Metalink`/`XML-RPC`/`Firefox3 Cookie`。
...
/// 降级拒绝方法的统一文案：LDownload 明确不支持该 aria2 能力。
pub(crate) fn err_unsupported(method: &str) -> String {
    format!("{method} is not supported by LDownload.")
}
...
mod tests {
    ...
    fn error_messages_match_aria2_wording() {
        ...
        assert_eq!(
            ...
            "aria2.shutdown is not supported by LDownload."
        );
    }
}
