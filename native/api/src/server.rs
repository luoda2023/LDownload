//! axum HTTP 服务器 —— LDownload 本机 API 服务。
//!
...
//! |---|---|---|---|
//! | 探活 | `GET /ping` | 总开关 | 无 |
//! | 脚本接管 | `POST /download`、`/download/batch` | `takeover_enabled` | `X-LDownload-Client` 头 + 可选 token |
...
/// ```
/// use std::collections::HashMap;
/// use ldown_api::server::ApiServerConfig;
///
...
pub struct ApiServerConfig {
    ...
    /// 与管理 API 共用 token 鉴权（Bearer / X-LDownload-Token）。
...
/// `Access-Control-Request-Headers`（缺省 `*`），使 `X-LDownload-Client` /
/// `X-LDownload-Token` / `Authorization` 等自定义头都能过检；额外带
...
#[utoipa::path(post, path = "/download", tag = "takeover",
    ...
    responses(
        ...
        (status = 401, description = "token 无效", body = crate::types::ResultMessage),
        (status = 403, description = "缺少 X-LDownload-Client 头", body = crate::types::ResultMessage),
    ),
...
#[utoipa::path(post, path = "/jsonrpc", tag = "aria2",
    responses((status = 200, description = "JSON-RPC 响应（错误在 envelope 内表达）。支持方法：aria2.addUri / aria2.getVersion / aria2.getGlobalStat / system.multicall / system.listMethods；token 可经 X-LDownload-Token 头或 params[0]=\"token:xxx\" 传递")),
    security(("tokenHeader" = []))
)]
...
/// X-LDownload-Token，复用管理 API 门禁）。请求返回 `200 application/json`
