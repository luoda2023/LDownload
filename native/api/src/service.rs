...
//! - 未来 headless server：实现为直接调用 `ldown_engine::Engine`
//!
...
pub trait ApiHost: Send + Sync {
    ...
    /// 读取全局配置表快照（config key → value，LDownload 原生键名）。
    ///
...
///
/// ```
/// use ldown_api::service::{TaskEventKind, task_event_for_transition};
///
/// // 首次观测即下载中 → Start。
...
pub struct TaskEvent {
    /// LDownload 任务 ID（UUID；jsonrpc 层负责转 GID）。
    pub task_id: String,
    /// 事件类别。
