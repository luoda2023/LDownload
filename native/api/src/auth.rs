...
//!    请求带 `X-LDownload-Client` 头。恶意网页用 `fetch()` 跨域携带自定义头会触发
...
//!    `X-LDownload-Token` 头，常量时间比较，作纵深防御。
//! 5. **管理 API 强制 token**：`/api/v1/*` 在 token 为空时一律拒绝（403），
//!    非空时要求 `Authorization: Bearer <token>` 或 `X-LDownload-Token` 匹配。
//! 6. **最终安全网**：接管/aria2 入口的下载都会在 LDownload 中弹出确认框，
...
/// 油猴脚本必须携带的来源标识头。
pub const CLIENT_HEADER: &str = "x-ldownload-client";
/// 可选鉴权 token 头。
pub const TOKEN_HEADER: &str = "x-ldownload-token";
...
/// `X-LDownload-Token` 头是否与配置 token 匹配（token 为空 = 不鉴权，恒通过）。
...
/// 变更类接管端点门禁：要求 `X-LDownload-Client` 头存在（挡跨域网页）+ token 校验。
///
/// 返回 `Err((状态码, 消息))`。
pub(crate) fn check_takeover_auth(
    ...
    let has_client = headers
        ...
        .unwrap_or(false);
    if !has_client {
        return Err((403, "missing X-LDownload-Client header"));
    }
...
/// 非空 → 接受 `Authorization: Bearer <token>` 或 `X-LDownload-Token`。
