...
/// `ldownload://` deep links, `magnet:`/`ed2k://` handlers, `.torrent`
/// association.
///
/// Levels differ on purpose: `ldownload://` is auto-registered and its absence
...
fn probe_shell_registration() -> Vec<DiagnosticCheck> {
    ...
    if crate::file_association::is_associated() {
        out.push(check(
            ...
            "",
            OK,
            ".torrent opens with LDownload".to_string(),
            "",
        ));
    } else {
        out.push(check(
...
fn probe_log_dir() -> DiagnosticCheck {
    let dir = ldown_engine::logger::log_dir();
...
fn environment_lines(nmh_exe: &str) -> Vec<String> {
    let log_dir = ldown_engine::logger::log_dir();
...
fn nmh_log_path() -> Option<PathBuf> {
    ...
    #[cfg(not(windows))]
    {
        ...
        let Some(home) = home else {
            return Some(Path::new("/tmp").join("ldown_nmh.log"));
        };
...
mod tests {
    ...
    async fn report_shape_is_within_the_wire_contract() {
        ...
        let schemes: Vec<&str> = report
            ...
            .collect();
        assert_eq!(schemes, ["ldownload", "magnet", "ed2k"]);
