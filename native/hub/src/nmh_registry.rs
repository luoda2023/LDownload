...
//!
//! Registers `com.ldownload.nmh` for Chrome, Edge, and Firefox so that the
//! browser extension can use `chrome.runtime.connectNative("com.ldownload.nmh")`
//! to communicate with the LDownload desktop app via the NMH relay binary.
//!
//! Registry keys (all HKCU — no admin required):
//!   Chrome:  `HKCU\Software\Google\Chrome\NativeMessagingHosts\com.ldownload.nmh`
//!   Edge:    `HKCU\Software\Microsoft\Edge\NativeMessagingHosts\com.ldownload.nmh`
//!   Firefox: `HKCU\Software\Mozilla\NativeMessagingHosts\com.ldownload.nmh`
//!
...
pub struct NmhTarget {
    ...
    pub label: String,
    /// 注册位置：Windows = `HKCU\Software\...\com.ldownload.nmh`；类 Unix = 清单文件绝对路径。
    pub location: String,
...
mod inner {
    ...
    const NMH_NAME: &str = "com.ldownload.nmh";
    const NMH_DESCRIPTION: &str = "LDownload Native Messaging Host";
    const NMH_EXE_NAME: &str = "ldown_nmh.exe";

    /// Manifest filename for Chrome/Edge (contains `allowed_origins`).
    const MANIFEST_FILENAME_CHROMIUM: &str = "com.ldownload.nmh.json";
    ...
    const MANIFEST_FILENAME_FIREFOX: &str = "com.ldownload.nmh.firefox.json";
    ...
    const FIREFOX_EXTENSION_ID: &str = "ldownload@ldownload.app";
    ...
    fn find_nmh_exe() -> Result<PathBuf, io::Error> {
        ...
        Err(io::Error::new(
            ...
            format!(
                "{} not found. Build it with: cargo build -p ldown_nmh",
                NMH_EXE_NAME
            ),
        ))
    }
    ...
    mod tests {
        ...
        ///
        /// 依赖真实 HKCU 注册表与已构建的 `ldown_nmh.exe`（`cargo build -p ldown_nmh`），
...
mod inner {
    ...
    const NMH_NAME: &str = "com.ldownload.nmh";
    const NMH_DESCRIPTION: &str = "LDownload Native Messaging Host";
    const NMH_EXE_NAME: &str = "ldown_nmh";
    ...
    const NMH_WRAPPER_NAME: &str = "ldown_nmh.sh";
    const MANIFEST_FILENAME_CHROMIUM: &str = "com.ldownload.nmh.json";
    const MANIFEST_FILENAME_FIREFOX: &str = "com.ldownload.nmh.json";
    const CHROME_EXTENSION_ID: &str = "chrome-extension://meleenglfggcmcajknpeeeiobnpfmahc/";
    ...
    /// Stable wrapper script path: ~/.local/share/ldownload/ldown_nmh.sh
    ///
    ...
    pub fn needs_update() -> bool {
        ...
        // browser installed after LDownload first registered — must trigger
...
mod inner {
    ...
    /// exec's the actual ldown_nmh binary, bypassing macOS AMFI's
    ...
    const NMH_WRAPPER_NAME: &str = "ldown_nmh.sh";
    const MANIFEST_FILENAME: &str = "com.ldownload.nmh.json";
    const CHROME_EXTENSION_ID: &str = "chrome-extension://meleenglfggcmcajknpeeeiobnpfmahc/";
    /// Edge Add-ons store extension ID — differs from Chrome (Edge ignores the
    /// manifest `key`) and must be whitelisted explicitly, else Edge store users
    /// get "forbidden" on connectNative → stuck on "未连接".
    ...
    /// the browser spawns `/bin/sh`, which in turn exec's `ldown_nmh`.
    ...
    pub fn needs_update() -> bool {
        ...
        // installed after LDownload first registered — must trigger re-register;
