...
/// 语义与字段约定见 `ldown_engine::events::EngineEvent::TaskCdnEvent`。
#[derive(Serialize, RustSignal)]
...
///
/// `scheme` is a bare scheme token (`"ldownload"` / `"ed2k"`) resolved against
...
/// Nested plugin info piece — mirrors `ldown_engine::plugin::PluginInfo`.
/// Hub-local: not shared with `ldown_api`'s `PluginDto` (hub→api is a
/// one-way dependency).
#[derive(Serialize, Deserialize, SignalPiece)]
...
#[cfg(hub_plugins)]
impl From<ldown_engine::plugin::PluginInfo> for PluginInfoSignal {
    fn from(info: ldown_engine::plugin::PluginInfo) -> Self {
        Self {
...
/// `ldown_engine::plugin::SettingField`. `min`/`max` use `f64` +
...
#[cfg(hub_plugins)]
impl From<ldown_engine::plugin::SettingField> for SettingFieldSignal {
    fn from(field: ldown_engine::plugin::SettingField) -> Self {
...
/// `select` widget option piece — mirrors `ldown_engine::plugin::SettingOption`.
#[derive(Serialize, Deserialize, SignalPiece)]
...
#[cfg(hub_plugins)]
impl From<ldown_engine::plugin::manifest::SettingOption> for SettingOptionSignal {
    fn from(opt: ldown_engine::plugin::manifest::SettingOption) -> Self {
        Self {
            value: opt.value,
            label: opt.label,
        }
...
/// Nested market entry piece — mirrors `ldown_engine::plugin::MarketEntry`.
...
#[cfg(hub_plugins)]
impl From<ldown_engine::plugin::MarketEntry> for MarketEntrySignal {
...
/// `source` mirrors `ldown_engine::components::FfmpegSource::as_str()`
...
pub struct FfmpegStatusReport {
    ...
    /// `ldown_engine::components::FfmpegStatus::managed_supported`). `false`
...
/// `source` mirrors `ldown_engine::components::ComponentSource::as_str()`
/// ("manual"/"managed"/"system"/"none").
#[derive(Serialize, RustSignal)]
pub struct YtdlpStatusReport {
    ...
    pub system_path: String,
    /// Whether managed install is available on this platform (mirrors
    /// `ldown_engine::components::YtdlpStatus::managed_supported`).
