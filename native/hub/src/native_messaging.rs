...
//!
//! Architecture:
//!   - LDownload main process creates a Named Pipe server at `\\.\pipe\ldownload`.
//!   - The NMH relay binary (`ldown_nmh.exe`) connects to this pipe.
...
use ldown_api::service::{ApiHost, LiveSpeed};
use ldown_api::types::{DownloadRequest, TaskDto};
...
mod server {
    ...
    /// mandatory integrity label so a Medium-IL `ldown_nmh.exe` (spawned by
    /// the browser) can connect even when LDownload runs elevated (High IL).
    ...
    /// connect even when LDownload runs elevated. Falls back to default security
...
mod server {
    ...
    /// - Linux:  `~/.local/share/ldownload/ldownload.sock`
    ...
    /// - Other Unix: `$XDG_RUNTIME_DIR/ldownload.sock` → `/tmp/ldownload.sock`
...
// ldown_api crate（native/api/src/types.rs 的所有者测试）。
