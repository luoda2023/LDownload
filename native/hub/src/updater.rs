...
//! `ldown_updater` helper binary that ships alongside the application:
//!
...
//! `ldown_updater` do not have the helper binary in their install directory.
...
const UPDATE_API_BASE: &str = "https://dicad.cn";
...
enum LinuxInstallType {
    ...
    AppImage,
    /// Installed via .deb package to /opt/ldownload/ (dpkg can locate the exe).
    Deb,
    /// Installed via .pkg.tar.zst to /opt/ldownload/ (pacman can locate the exe).
    Arch,
    /// Extracted tar.gz in any user-writable directory.
    Portable,
}

/// Detect how LDownload was installed on this Linux system.
#[cfg(target_os = "linux")]
fn detect_linux_install_type() -> LinuxInstallType {
    ...
    // 2. System package: both deb and arch install to /opt/ldownload/.
    if exe_str.starts_with("/opt/ldownload") {
...
/// identifiers. Only the subset LDownload emits (`X.Y.Z` / `X.Y.Z-pre`) is used.
struct SemVer {
    core: (u64, u64, u64),
    pre: Vec<PreId>,
}
...
fn pick_download_dir() -> PathBuf {
    #[cfg(target_os = "macos")]
    {
        // 系统 API 解析（`ldown_engine::user_dirs`），不拼 `$HOME/Downloads`。
        if let Some(downloads) = ldown_engine::user_dirs::download_dir()
            && (downloads.is_dir()
...
/// constant of the same purpose in `ldown_updater`.
...
fn ensure_install_dir_writable() -> Result<(), UpdateError> {
    ...
    let probe = dir.join(format!(".ldownload_write_test_{}", std::process::id()));
    match std::fs::File::create(&probe) {
        ...
        Err(e) => Err(UpdateError::Other(format!(
            "The install folder is not writable, so the update cannot replace \
             ...
             happens when LDownload is in a protected location such as \
             \"Program Files\" or on a read-only drive. Move LDownload to a \
             normal folder (e.g. your user directory) and try again, or download \
             the latest version from https://dicad.cn\n\n({e})",
            dir.display()
        ))),
    }
}
...
fn install_macos_dmg(dmg_path: &str) -> Result<(), UpdateError> {
    ...
    // (which typically contains LDownload.app + an Applications symlink).
...
/// macOS update: spawn `ldown_updater` and exit immediately.
///
...
///
/// The updater binary lives at `LDownload.app/Contents/MacOS/ldown_updater`,
...
fn install_macos_app(tarball_path: &str) -> Result<(), UpdateError> {
    ...
    // LDownload.app/Contents/MacOS/ldownload
    //              ↑ parent  ↑ parent  ↑ parent  →  LDownload.app
    let app_bundle = exe
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
    ...
    // Parent of LDownload.app  →  /Applications (or wherever the user placed it)
...
/// Locate the `ldown_updater[.exe]` helper binary that is shipped alongside
...
fn find_updater_bin() -> Result<PathBuf, UpdateError> {
    ...
    #[cfg(target_os = "windows")]
    let name = "ldown_updater.exe";
    #[cfg(not(target_os = "windows"))]
    let name = "ldown_updater";
...
fn bootstrap_updater_from_zip(zip_path: &str) -> Result<PathBuf, UpdateError> {
    ...
    const HELPER_NAME: &str = "ldown_updater.exe";
    ...
    for i in 0..archive.len() {
        ...
        if file_name.eq_ignore_ascii_case(HELPER_NAME) {
            let dest = std::env::temp_dir()
                .join(format!("ldown_updater_boot_{}.exe", std::process::id()));
    ...
    Err(UpdateError::Other(format!(
        "{HELPER_NAME} was not found inside the downloaded archive. \
         ...
         Please download and extract the new version manually from https://dicad.cn"
    )))
}
...
/// Windows setup update: spawn `ldown_updater` and exit immediately.
///
...
/// Windows portable update: spawn `ldown_updater` and exit immediately.
///
/// The updater uses `WaitForSingleObject` for precise OS-level process-exit
/// detection (no polling, no tasklist), then:
///   1. Renames itself aside so the ZIP can overwrite `ldown_updater.exe`.
...
///
/// Cold-start migration: if `ldown_updater.exe` is absent (upgrade from an
...
fn install_portable(zip_path: &str) -> Result<(), UpdateError> {
    ...
    // an older version that did not ship ldown_updater.exe.
...
/// Linux AppImage update: spawn `ldown_updater` and exit immediately.
///
...
/// Linux deb update: spawn `ldown_updater` and exit immediately.
///
...
/// Linux Arch update: spawn `ldown_updater` and exit immediately.
///
...
/// Linux portable tar.gz update: spawn `ldown_updater` and exit immediately.
///
...
mod tests {
    ...
    fn sanitize_strips_query_and_fragment() {
        assert_eq!(
            sanitize_filename("LDownload-0.2.0-windows-x64-setup.exe?source=github"),
            "LDownload-0.2.0-windows-x64-setup.exe"
        );
        ...
        assert_eq!(sanitize_filename("?only=query"), "LDownload-update");
    }
