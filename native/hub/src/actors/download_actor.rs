...
use ldown_engine::db::Db;
use ldown_engine::download_manager::{
    self, CreateGroupSpec, GroupItemSpec, NewTaskSpec, ResolvePreviewOutcome, TaskDone,
};
use ldown_engine::events::EventSink;
#[cfg(hub_plugins)]
use ldown_engine::plugin::{PluginError, PluginManager};
use ldown_engine::proxy_config::ProxyConfig;
use ldown_engine::selection::HostSelection;
use ldown_engine::{Engine, EngineConfig};
...
use crate::updater;
use ldown_api::server::{ApiServerConfig, ApiServerHandle, spawn_api_server};
use ldown_api::service::TaskEvent;

/// Compute default save directory (platform-dependent).
///
/// 桌面端一律由系统 API 给出（`ldown_engine::user_dirs`）：Windows 读已知
...
pub(crate) fn default_save_dir() -> String {
    ...
    {
        if let Some(pkg) = ldown_engine::data_dir::android_package_name() {
            return format!("/storage/emulated/0/Android/data/{pkg}/files/Download");
        }
    }
    ldown_engine::user_dirs::download_dir_or_cwd()
}
        ...
        seed_limit_operator: cfg
            ...
            .map(|v| {
                if v.eq_ignore_ascii_case("and") {
                    ldown_engine::bt_seeding::SeedingLimitOperator::And
                } else {
                    ldown_engine::bt_seeding::SeedingLimitOperator::Or
                }
            })
            .unwrap_or(ldown_engine::bt_seeding::SeedingLimitOperator::Or),
...
fn spawn_tracker_sub_refresh(
    db: Db,
    tx: mpsc::Sender<ldown_engine::tracker_subscription::FetchOutcome>,
) {
    tokio::spawn(async move {
        ...
        let urls = cfg
            ...
            .cloned()
            .unwrap_or_else(ldown_engine::tracker_subscription::default_subscription_urls);
        let outcome = ldown_engine::tracker_subscription::fetch_subscriptions(&urls).await;
...
fn spawn_ed2k_server_sub_refresh(
    db: Db,
    tx: mpsc::Sender<ldown_engine::ed2k::server_subscription::ServerFetchOutcome>,
) {
    tokio::spawn(async move {
        let cfg = db.get_all_config().await.unwrap_or_default();
        let urls = cfg
            ...
            .cloned()
            .unwrap_or_else(ldown_engine::ed2k::server_subscription::default_server_met_urls);
        let outcome =
            ldown_engine::ed2k::server_subscription::fetch_server_subscriptions(&urls).await;
        if outcome.is_success() {
            let now = chrono::Utc::now().timestamp();
            if let Err(e) = db
            ...
            if let Err(e) = db
                .set_config(
                    ...
                    &ldown_engine::ed2k::server_subscription::CACHE_FORMAT_VERSION.to_string(),
                )
                .await
            {
                log_info!(
...
fn spawn_ed2k_nodes_dat_refresh(db: Db) {
    tokio::spawn(async move {
        ...
        match ldown_engine::ed2k::kad::fetch_nodes_dat(&url).await {
            Ok(bytes) => {
...
pub async fn run(db_dir: PathBuf) {
    ...
    if bt_config.custom_trackers.trim().is_empty() {
        let defaults = ldown_engine::bt_downloader::default_tracker_list();
    ...
    let mut engine = match Engine::new(
        EngineConfig {
            ...
            // `ldown_engine::data_dir::resolve_data_dir(None)` 解析,
    ...
    let mut resolve_rx: mpsc::UnboundedReceiver<
        ldown_engine::download_manager::ResolveOutcome,
    ...
    let (tracker_sub_tx, mut tracker_sub_rx) =
        mpsc::channel::<ldown_engine::tracker_subscription::FetchOutcome>(4);
    ...
    // 启动时自动刷新：订阅启用且缓存超过 24 小时未更新。
    {
        ...
        if sub_enabled
            && now.saturating_sub(updated_at)
                > ldown_engine::tracker_subscription::REFRESH_INTERVAL_SECS
        {
            log_info!(
    ...
    let (ed2k_sub_tx, mut ed2k_sub_rx) =
        mpsc::channel::<ldown_engine::ed2k::server_subscription::ServerFetchOutcome>(4);
    ...
    // 启动时自动刷新：订阅启用且缓存超过 24 小时未更新。
    {
        ...
        let cache_version = cfg
            ...
            .unwrap_or(0);
        let version_stale =
            cache_version < ldown_engine::ed2k::server_subscription::CACHE_FORMAT_VERSION;
        if version_stale {
            log_info!(
                "[actor] ed2k server sub cache version {} < {}, invalidating (byte-order fix)",
                cache_version,
                ldown_engine::ed2k::server_subscription::CACHE_FORMAT_VERSION
            );
        ...
        if sub_enabled
            && (version_stale
                || now.saturating_sub(updated_at)
                    > ldown_engine::ed2k::server_subscription::REFRESH_INTERVAL_SECS)
        {
            log_info!(
    ...
    let (ext_dl_tx, mut native_msg_rx) =
        mpsc::channel::<Vec<ldown_api::types::DownloadRequest>>(64);
    ...
    #[cfg(hub_link)]
    let link_mgr: Option<Arc<ldown_engine::link::LinkManager>> = {
        let self_name = std::env::var("COMPUTERNAME")
            .or_else(|_| std::env::var("HOSTNAME"))
            .ok()
            .filter(|s| !s.trim().is_empty())
            .unwrap_or_else(|| "LDownload".to_string());
        let self_info = ldown_engine::link::SelfInfo {
            name: self_name,
            platform: Some(std::env::consts::OS.to_string()),
            app_version: None,
        };
        let (link_tx, mut link_rx) = mpsc::channel::<ldown_engine::link::LinkEngineEvent>(64);
        ...
        let api_port = engine
            ...
            .unwrap_or(17800);
        match ldown_engine::link::LinkManager::load(
            engine.db.clone(),
            self_info,
            api_port,
            link_tx,
    ...
    #[cfg(hub_plugins)]
    let api_host: Arc<dyn ldown_api::service::ApiHost> = Arc::new(HubApiHost::new(
        engine.db.clone(),
        api_cmd_tx,
        ext_dl_tx.clone(),
        live_speeds,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        tokio::task::spawn_blocking(move || {
            if !protocol_registry::is_registered(protocol_registry::LDOWNLOAD) {
                if let Err(e) = protocol_registry::register(protocol_registry::LDOWNLOAD) {
                    log_info!("[actor] auto-register ldownload:// protocol failed: {}", e);
                }
            } else {
                log_info!("[actor] ldownload:// protocol already registered");
            }
    ...
    struct ExtRequestCtx {
        ...
        body: Option<ldown_api::types::RequestBody>,
        cookies: String,
        referrer: String,
    ...
    enum RssSignal {
        ...
        Tick,
        /// off-actor 抓取/验证回流。
        Engine(Box<ldown_engine::rss::RssEvent>),
    }
    enum WebhookSignal {
    ...
    let rss_tx = aux_tx;
    {
        ...
        let mut seeding_interval =
            tokio::time::interval(ldown_engine::bt_seeding::SEEDING_EVAL_INTERVAL);
    ...
    loop {
        tokio::select! {
                ...
                AuxSignal::Webhook(webhook_signal) => match webhook_signal {
                    ...
                    WebhookSignal::Test(msg) => {
                        ...
                        tokio::spawn(async move {
                            ...
                            let spec: ldown_engine::webhook::EndpointSpec =
            ...
            // --- URL protocol signals (ldownload:// deep links, ed2k:// links) ---
            ...
            Some(_) = detect_sys_proxy_recv.recv() => {
                tokio::task::spawn_blocking(|| {
                    match ldown_engine::proxy_config::detect_system_proxy() {
                        Ok(Some(cfg)) => {
                            SystemProxyInfo {
                                detected: true,
            ...
            Some(_) = req_ffmpeg_status_recv.recv() => {
                let status =
                    ldown_engine::components::ffmpeg_status(&engine.db, &engine.data_dir).await;
            ...
            Some(_) = req_ffmpeg_versions_recv.recv() => {
                ...
                match ldown_engine::downloader::build_client(&proxy_cfg, "") {
                    Ok(client) => {
                        tokio::spawn(async move {
                            match ldown_engine::components::list_versions(&client).await {
                                Ok(v) => {
                                    FfmpegVersionList {
                                        ok: true,
                                        message: String::new(),
                ...
                match ldown_engine::downloader::build_client(&proxy_cfg, "") {
                    Ok(client) => {
                        ...
                        tokio::spawn(async move {
                            ...
                            let result = ldown_engine::components::install_ffmpeg(
                                &db,
                                &data_dir,
                                &client,
                                version_opt,
                            ...
                            // 回退状态（如此前已有的托管版本仍然生效）。
                            let status =
                                ldown_engine::components::ffmpeg_status(&db, &data_dir).await;
                            ffmpeg_status_report(status).send_signal_to_dart();
                        });
                    }
                    Err(e) => {
            ...
            Some(_) = uninstall_ffmpeg_recv.recv() => {
                let result =
                    ldown_engine::components::uninstall_ffmpeg(&engine.db, &engine.data_dir)
                        .await;
                match result {
                    Ok(()) => {
                        FfmpegInstallResult { ok: true, message: String::new() }
            ...
            Some(_) = req_ytdlp_status_recv.recv() => {
                let status =
                    ldown_engine::components::ytdlp_status(&engine.db, &engine.data_dir).await;
            ...
            Some(_) = req_ytdlp_versions_recv.recv() => {
                ...
                match ldown_engine::downloader::build_client(&proxy_cfg, "") {
                    Ok(client) => {
                        tokio::spawn(async move {
                            match ldown_engine::components::list_ytdlp_versions(&client).await {
                                Ok(v) => {
                                    YtdlpVersionList {
                                        ok: true,
                                        message: String::new(),
                ...
                match ldown_engine::downloader::build_client(&proxy_cfg, "") {
                    Ok(client) => {
                        ...
                        tokio::spawn(async move {
                            ...
                            let result = ldown_engine::components::install_ytdlp(
                                &db,
                                &data_dir,
                                &client,
                                version_opt,
                            ...
                            let status =
                                ldown_engine::components::ytdlp_status(&db, &data_dir).await;
                            ytdlp_status_report(status).send_signal_to_dart();
                        });
                    }
                    Err(e) => {
            ...
            Some(_) = uninstall_ytdlp_recv.recv() => {
                let result =
                    ldown_engine::components::uninstall_ytdlp(&engine.db, &engine.data_dir)
                        .await;
                match result {
                    Ok(()) => {
                        YtdlpInstallResult { ok: true, message: String::new() }
...
/// `failed_key` 恒为空——`ldown_engine::plugin::PluginError` 未暴露结构化
...
/// 依赖表见 `ldown_engine::plugin::dependencies`。
async fn plugin_missing_components(
    plugin_manager: &PluginManager,
    db: &ldown_engine::db::Db,
    data_dir: &Path,
    identity: &str,
) -> Vec<String> {
    let perms = plugin_manager.permissions_of(identity).await;
    ldown_engine::plugin::dependencies::missing_components(db, data_dir, &perms).await
}
...
/// `ldown_engine::components::FfmpegStatus` → `FfmpegStatusReport` 信号。
/// `source` 走 `FfmpegSource::as_str()` 保持与 server/web 端一致的 wire 字符串。
fn ffmpeg_status_report(status: ldown_engine::components::FfmpegStatus) -> FfmpegStatusReport {
    FfmpegStatusReport {
...
/// `ldown_engine::components::YtdlpStatus` → `YtdlpStatusReport` 信号。
/// `source` 走 `ComponentSource::as_str()` 保持与 server/web 端一致的 wire 字符串。
fn ytdlp_status_report(status: ldown_engine::components::YtdlpStatus) -> YtdlpStatusReport {
    YtdlpStatusReport {
        source: status.source.as_str().to_string(),
        path: status.path,
        version: status.version,
...
async fn handle_api_command(
    cmd: ApiCommand,
    engine: &mut Engine,
    tracker_sub_tx: &mpsc::Sender<ldown_engine::tracker_subscription::FetchOutcome>,
    ed2k_sub_tx: &mpsc::Sender<ldown_engine::ed2k::server_subscription::ServerFetchOutcome>,
    api_host: &Arc<dyn ldown_api::service::ApiHost>,
...
fn send_webhook_snapshot(engine: &Engine) {
    ...
    .send_signal_to_dart();
    WebhookPresets {
        presets: ldown_engine::webhook::preset_catalog()
            .into_iter()
            .map(Into::into)
            .collect(),
        variables: ldown_engine::webhook::TEMPLATE_VARIABLES
            .iter()
            .map(|v| (*v).to_string())
            .collect(),
    }
...
async fn apply_config_key(
    ...
    match key {
        ...
        ldown_engine::webhook::CONFIG_KEY_ENDPOINTS => {
...
fn synthesize_batch_request(
    reqs: &[ldown_api::types::DownloadRequest],
) -> ExternalDownloadRequest {
...
/// (ldown_api 是对外 wire 契约，engine 侧不感知传输层)。
fn nm_body_to_captured(
    body: ldown_api::types::RequestBody,
) -> ldown_engine::downloader::CapturedRequestBody {
    use ldown_api::types::RequestBody;
    use ldown_engine::downloader::CapturedRequestBody as Captured;
    match body {
...
#[cfg(hub_link)]
async fn emit_link_devices(link: &ldown_engine::link::LinkManager) {
    use rinf::RustSignal;
...
/// 把引擎侧 [`LinkEngineEvent`](ldown_engine::link::LinkEngineEvent) 转成
/// Dart 信号（发现/配对成功/解除配对/错误）。
#[cfg(hub_link)]
fn emit_link_engine_event(ev: ldown_engine::link::LinkEngineEvent) {
    use ldown_engine::link::{DiscoveryKind, LinkEngineEvent as E};
    use rinf::RustSignal;
    match ev {
        E::Discovered(p) => {
...
async fn handle_link_command(
    ...
    link: Arc<ldown_engine::link::LinkManager>,
) {
    use rinf::RustSignal;
    ...
    match msg.action.as_str() {
        ...
        "probe" => match link.probe(&msg.host, msg.port as u16).await {
            Ok(p) => emit_link_engine_event(ldown_engine::link::LinkEngineEvent::Discovered(p)),
...
mod tests {
    ...
    fn req(url: &str) -> ldown_api::types::DownloadRequest {
