...
//! ```text
//! HKCU\Software\Classes\.torrent                               → "LDownload.TorrentFile"
//! HKCU\Software\Classes\LDownload.TorrentFile                   → "BitTorrent File"
//! HKCU\Software\Classes\LDownload.TorrentFile\DefaultIcon       → "<exe>,0"
//! HKCU\Software\Classes\LDownload.TorrentFile\shell\open\command → "\"<exe>\" \"%1\""
//! ```
//!
...
//! `installer/windows/setup.iss` removes any LDownload-owned leftovers
...
mod inner {
    ...
    const PROG_ID: &str = "LDownload.TorrentFile";
    ...
    /// Check whether `.torrent` files are currently associated with LDownload.
    ///
    /// Returns `true` if `HKCU\Software\Classes\.torrent` default value
    /// equals `"LDownload.TorrentFile"`. We intentionally do NOT compare the
    ...
    /// confirm LDownload owns the association.
    pub fn is_associated() -> bool {
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);

        // Check .torrent → LDownload.TorrentFile
        let ext_key =
    ...
    pub fn associate() -> Result<(), io::Error> {
        ...
        log_info!("[file_assoc] associated .torrent with LDownload (exe={exe})");
        Ok(())
    }

    /// Remove `.torrent` file association for LDownload.
    pub fn disassociate() -> Result<(), io::Error> {
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);

        // Only remove if currently associated to us (don't break other app's association)
        if !is_associated() {
            log_info!("[file_assoc] not associated to LDownload, skipping removal");
            return Ok(());
        }
        ...
        // Remove LDownload.TorrentFile tree
...
mod inner {
    ...
    /// Register LDownload as the default handler for `.torrent` files.
    ///
    /// Requires that `com.ldownload.app.desktop` is already installed in an
    /// XDG applications directory (handled by the package installer).
    pub fn associate() -> Result<(), io::Error> {
        std::process::Command::new("xdg-mime")
            .args([
                "default",
                "com.ldownload.app.desktop",
    ...
    /// Remove LDownload as the default handler for `.torrent` files by
    ...
    /// remove the `application/x-bittorrent=com.ldownload.app.desktop` line
    ...
    pub fn disassociate() -> Result<(), io::Error> {
        ...
        let filtered: Vec<&str> = lines
            ...
            .filter(|l| {
                ...
                !(lower.starts_with("application/x-bittorrent=") && lower.contains("ldownload"))
            })
            .map(|l| l.as_str())
            .collect();
...
mod inner {
    ...
    pub fn associate() -> Result<(), io::Error> {
        ...
        log_info!("[file_assoc] associated .torrent with LDownload (bundle={bundle_id})");
        Ok(())
    }

    /// Remove LDownload as the default handler for `.torrent` files.
    ///
