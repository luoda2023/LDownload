//! [`HubApiHost`] —— `ldown_api::service::ApiHost` 的桌面 App 实现。
//!
//! ## 读写分离
//!
...
#[cfg(hub_plugins)]
use ldown_api::types::{MarketEntryDto, PluginDto};
use ldown_engine::db::Db;
use ldown_engine::download_manager::{CreateGroupSpec, GroupItemSpec, ResolvePreviewOutcome};
#[cfg(hub_link)]
use ldown_engine::link::{DiscoveredPeer, DiscoveryKind, LinkError, WireHello};
#[cfg(hub_plugins)]
...
pub enum ApiCommand {
    ...
    RssCreate {
        source: Box<ldown_engine::rss::model::RssSourceInfo>,
        ack: oneshot::Sender<Option<String>>,
    },
    ...
    RssValidate {
        ...
        user_agent: String,
        proxy_url: String,
        ack: oneshot::Sender<Box<ldown_engine::rss::RssValidateOutcome>>,
    },
}

/// 桌面 App 的 API 宿主。构造后传给 `ldown_api::server::spawn_api_server`。
pub struct HubApiHost {
    ...
    #[cfg(hub_link)]
    link: Option<Arc<ldown_engine::link::LinkManager>>,
}
...
impl HubApiHost {
    ...
    pub fn new(
        ...
        data_dir: PathBuf,
        #[cfg(hub_link)] link: Option<Arc<ldown_engine::link::LinkManager>>,
    ) -> Self {
        Self {
            db,
            cmd_tx,
...
impl ApiHost for HubApiHost {
    ...
    async fn update_rss_source(&self, source_id: &str, req: RssSourceDto) -> Result<(), ApiError> {
        let mut source: ldown_engine::rss::model::RssSourceInfo = req.into();
    ...
    async fn list_rss_items(&self, source_id: &str) -> Result<Vec<RssItemDto>, ApiError> {
        self.db
            .load_rss_items(source_id, ldown_engine::rss::MAX_ITEMS_PER_SOURCE)
            .await
