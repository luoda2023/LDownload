...
//!   「前态 → 新状态」的迁移判定(见 [`ldown_api::service::task_event_for_transition`])经
...
pub struct RinfEventSink {
    ...
    /// `task_id → 上一次已知状态码`,供
    /// [`ldown_api::service::task_event_for_transition`] 判定状态
...
impl RinfEventSink {
    ...
    /// [`ldown_api::service::task_event_for_transition`] 判定为不发,
    /// 不会重复触发一次 `Error`。
    ///
...
mod tests {
    ...
    use ldown_api::service::{LiveSpeed, TaskEventKind};
    use ldown_engine::events::{EngineEvent, EventSink};
    use tokio::sync::broadcast;
