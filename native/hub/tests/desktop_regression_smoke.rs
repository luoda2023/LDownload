//! 桌面回归冒烟测试 —— `docs/ldownload-engine-decouple-plan.md` Verification
//! 条款 6。
//!
//! **范围调整说明**：原条款要求"迁移前后对比"，但 hub → `ldown_engine`
...
//! （`ldown_engine::Engine::new` → `DownloadManager::create_task` →
...
use ldown_engine::{Engine, EngineConfig, NoopSelection};
...
/// 端到端验证：hub 使用的 `ldown_engine::Engine` 构造路径
...
async fn desktop_regression_smoke() {
    ...
    let work_dir = std::env::temp_dir().join(format!(
        "ldownload_desktop_regression_smoke_{}",
        std::process::id()
    ));
    ...
    tokio::spawn(ldown_engine::download_manager::progress_reporter(
        progress_rx,
        engine.db.clone(),
        sink.clone(),
    ));
