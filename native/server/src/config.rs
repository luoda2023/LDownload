...
//! | 环境变量 | 含义 | 默认 |
//! |---|---|---|
//! | `LDOWNLOAD_DATA_DIR` | 数据目录（DB/日志） | 平台自动探测 |
//! | `LDOWNLOAD_DATABASE_URL` | 数据库连接 URL（`sqlite:`/`postgres:`） | 数据目录下 SQLite |
//! | `LDOWNLOAD_BIND` | HTTP 监听地址 | `0.0.0.0:17800` |
//! | `LDOWNLOAD_WEBROOT` | 覆盖内嵌 Web UI，改从该磁盘目录托管 SPA | 未设置（用二进制内嵌的前端） |
//! | `LDOWNLOAD_TOKEN` | 预置管理访问密钥（仅在库中尚未设置时采纳） | 未设置（走 Web 向导） |
//! | `LDOWNLOAD_DEMO` | 演示模式：仅允许下载内置本地演示文件 | 未设置（关闭） |
//! | `LDOWNLOAD_DEMO_URL` | 演示模式：仅允许下载该 URL（覆盖内置） | 未设置（关闭） |
//! | `LDOWNLOAD_LANG` | Web UI 默认语言（`en`/`zh`），设置页保存过语言后以保存值为准 | 未设置（回退浏览器语言） |
//!
...
pub struct ServerConfig {
    ...
    /// 显式设置 `LDOWNLOAD_WEBROOT` 时出现，用于自定义/调试前端。
...
impl ServerConfig {
    pub fn from_env() -> Self {
        let bind = std::env::var("LDOWNLOAD_BIND").unwrap_or_else(|_| "0.0.0.0:17800".to_string());
        let data_dir_override = std::env::var_os("LDOWNLOAD_DATA_DIR").map(PathBuf::from);
        let database_url = std::env::var("LDOWNLOAD_DATABASE_URL")
            .ok()
            .filter(|s| !s.trim().is_empty());
        let webroot = std::env::var_os("LDOWNLOAD_WEBROOT")
            .map(PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty());
        let demo_url = std::env::var("LDOWNLOAD_DEMO_URL")
            .ok()
            .as_deref()
            .and_then(parse_demo_url)
            .or_else(|| demo_flag_enabled().then(|| builtin_demo_url(&bind)));
        let language = match std::env::var("LDOWNLOAD_LANG") {
            Ok(raw) => {
                let lang = parse_lang(&raw);
                if lang.is_none() && !raw.trim().is_empty() {
                    eprintln!("LDOWNLOAD_LANG 无法识别（支持 en / zh），已忽略：{raw}");
                }
                lang
            }
            Err(_) => None,
...
/// 归一化 `LDOWNLOAD_LANG`：剥首尾空白与包裹引号后取 BCP 47 主语言子标签
...
/// 归一化 `LDOWNLOAD_DEMO_URL`：去掉首尾空白与误带的包裹引号
...
/// `LDOWNLOAD_DEMO` 是否为真值（`1`/`true`/`yes`/`on`，忽略大小写）。
fn demo_flag_enabled() -> bool {
    std::env::var("LDOWNLOAD_DEMO")
        .map(|v| flag_truthy(&v))
        .unwrap_or(false)
}
...
/// [`ldown_api::auth::check_management_auth`]），Web 界面会进入
/// 「设置访问密钥」向导（`POST /api/v1/setup`）。
///
/// 不再自动生成 token 并打印到 stderr：NAS（群晖/QNAP/Unraid）用户拿不到
/// 容器/套件的 stderr，一次性打印的密钥等于永久锁在门外。无人值守部署
/// （docker-compose / k8s / CI）可用 `LDOWNLOAD_TOKEN` 预置密钥跳过向导。
pub async fn ensure_server_config(db: &Db) -> Result<String, ldown_engine::db::DbError> {
    ...
    let Some(preset) = std::env::var("LDOWNLOAD_TOKEN")
        .ok()
    ...
    if let Err(why) = validate_access_key(&preset) {
        log_info!("[server] LDOWNLOAD_TOKEN rejected: {}", why);
        eprintln!("LDOWNLOAD_TOKEN 不符合密钥要求（{why}），已忽略；请在 Web 界面完成首次设置。");
        return Ok(String::new());
    }
    db.set_config("local_server_token", &preset).await?;
    log_info!("[server] management token adopted from LDOWNLOAD_TOKEN");
    Ok(preset)
}
...
pub fn print_setup_banner(bind: &str) {
    let port = bind.rsplit(':').next().unwrap_or("17800");
    eprintln!("==============================================================");
    eprintln!("  LDownload Server: first run — no access key is set yet.");
    eprintln!("  Open the Web UI and create one:");
    eprintln!("    http://<server-ip>:{port}/");
    eprintln!("  Requirements: 8+ characters, letters and digits.");
    eprintln!("  Unattended deploys can preset it via LDOWNLOAD_TOKEN.");
    eprintln!("  ---");
