...
//!
//! 与 `ldown_api::types` 同理：**不直接序列化** `EngineEvent` /
...
use ldown_api::types::{GroupDto, QueueDto, RssItemDto, RssSourceDto, TaskDto};
use ldown_engine::components::{FfmpegStatus, FfmpegVersions, YtdlpStatus, YtdlpVersions};
use ldown_engine::model::{
    BtFileEntry, CdnNodeInfo, HlsQualityOption, QueuePosition, ResolveVariantOption, SegmentDetail,
};
use ldown_engine::webhook::{PresetInfo, WebhookDelivery};
...
pub enum WsServerMsg {
    ...
    /// `ldown_engine::events::EngineEvent::TaskCdnEvent`。
    TaskCdnEvent {
        task_id: String,
...
pub struct StatsResponse {
    ...
    /// 演示模式开关（服务器以 `LDOWNLOAD_DEMO_URL` 启动时为 true）。
    pub demo_mode: bool,
...
mod tests {
    ...
    fn sample_queue_dto(id: &str) -> QueueDto {
        QueueDto {
            ...
            position: 1,
            default_segments: 4,
            default_user_agent: "LDownload/1.0".into(),
            is_running: true,
