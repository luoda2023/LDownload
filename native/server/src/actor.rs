...
use ldown_api::service::ApiError;
use ldown_api::types::CreateTaskRequest;
use ldown_engine::Engine;
...
use ldown_engine::rss::RssValidateOutcome;
...
pub enum ActorCmd {
    ...
    WebhookDeliveries {
        ack: oneshot::Sender<Vec<ldown_engine::webhook::WebhookDelivery>>,
    },
    /// 清空投递日志。
    WebhookClear {
        ack: oneshot::Sender<()>,
    ...
    WebhookTest {
        ...
        ack: oneshot::Sender<Box<ldown_engine::webhook::WebhookDelivery>>,
    },
}
...
async fn apply_config(engine: &mut Engine, keys: &[String]) {
    ...
    for key in keys {
        match key.as_str() {
            ...
            k if k == ldown_engine::webhook::CONFIG_KEY_ENDPOINTS => {
                if let Some(v) = all.get(key) {
...
) -> ldown_engine::tracker_subscription::FetchOutcome {
...
pub async fn refresh_ed2k_server_sub(
    db: &Db,
) -> ldown_engine::ed2k::server_subscription::ServerFetchOutcome {
    let cfg = db.get_all_config().await.unwrap_or_default();
    let urls = cfg
...
mod tests {
    ...
    fn ed2k_server_sub_startup_plan_covers_version_staleness_freshness_and_opt_out() {
        const CUR: i64 = ldown_engine::ed2k::server_subscription::CACHE_FORMAT_VERSION;
        const INTERVAL: i64 = ldown_engine::ed2k::server_subscription::REFRESH_INTERVAL_SECS;
