...
//!   （复用 [`ldown_api::auth::check_management_auth`]）。
...
//!   落定后 `setup` 立刻转为 409。部署方若在意抢注，用 `LDOWNLOAD_TOKEN` 预置。
...
use ldown_api::auth::{TokenCell, check_management_auth, constant_time_eq};
use ldown_api::service::ApiError;
use ldown_api::types::{QueueDto, TaskDto};
use ldown_engine::components::{
...
use ldown_engine::downloader::build_client;
...
/// 组装全部扩展路由（含鉴权中间件），与 `ldown_api::server::api_router`
...
/// 统一 JSON 错误体（与 `ldown_api` 的 `ResultMessage` 形态一致）。
...
async fn logs_info() -> Result<Response, ApiError> {
    let dir = ldown_engine::logger::log_dir().display().to_string();
    let files = ldown_engine::logger::list_log_files()
        .into_iter()
        .map(|m| LogFileDto {
            name: m.name,
            size: m.size as i64,
...
#[utoipa::path(get, path = "/api/v1/logs/export", tag = "server",
    params(("token" = String, Query, description = "管理 token")),
    responses(
        (status = 200, description = "日志压缩包（attachment: ldownload_logs.zip）"),
...
async fn logs_export(State(state): State<ServerState>, Query(q): Query<TokenQuery>) -> Response {
    ...
    let bytes = match ldown_engine::logger::export_logs_zip() {
        Ok(b) => b,
    ...
    if let Ok(v) = header::HeaderValue::from_str(&content_disposition("ldownload_logs.zip")) {
        headers.insert(header::CONTENT_DISPOSITION, v);
    }
    response
}
...
/// 本 crate 扩展端点的 OpenAPI 文档（与 `ldown_api::openapi::ApiDoc` 合并）。
#[derive(OpenApi)]
#[openapi(
    info(
        title = "LDownload Server API",
        description = "LDownload headless 服务器 HTTP API：核心任务管理（复用桌面 API 契约）\
...
fn merged_openapi_json() -> &'static str {
    ...
    SPEC.get_or_init(|| {
        let merged = ServerApiDoc::openapi().merge_from(ldown_api::openapi::ApiDoc::openapi());
        merged
            .to_pretty_json()
...
<html lang="zh-CN">
  <head>
    ...
    <title>LDownload Server API Docs</title>
