//! [`ServerApiHost`] —— `ldown_api::service::ApiHost` 的 headless 实现。
//!
...
use ldown_api::service::{ApiError, ApiHost, LiveSpeed, TaskEvent};
use ldown_api::types::{
...
use ldown_engine::download_manager::{CreateGroupSpec, GroupItemSpec};
use ldown_engine::link::{DiscoveredPeer, DiscoveryKind, LinkError, LinkManager, WireHello};
use ldown_engine::plugin::{MarketClient, PluginManager};
use ldown_engine::rss::MAX_ITEMS_PER_SOURCE;
...
pub struct ServerApiHost {
    ...
    hub: Arc<WsHub>,
    /// 演示模式：`Some(url)` 时仅允许下载该 URL（`LDOWNLOAD_DEMO_URL`）。
    demo_url: Option<String>,
    /// 部署级默认 Web UI 语言（`LDOWNLOAD_LANG`）。仅作 [`ApiHost::web_language`]
...
impl ApiHost for ServerApiHost {
    ...
    /// （LDownload 原生 key，aria2 选项名翻译在 jsonrpc 层完成）。
    ...
    /// 时回退 `LDOWNLOAD_LANG`。每次请求现读 DB，语言变更无需重启即生效。
    ...
    async fn plugin_missing_components(&self, identity: &str) -> Vec<String> {
        ...
        ldown_engine::plugin::dependencies::missing_components(&self.db, &self.data_dir, &perms)
            .await
    }
...
/// 两个 crate（`ldown_api` 不依赖引擎的可选 link 模块），这里做一次显式搬运。
fn map_confirm_outcome(
    outcome: ldown_engine::link::PairConfirmOutcome,
) -> LinkPairConfirmOutcome {
    use ldown_engine::link::PairConfirmOutcome as E;
    match outcome {
...
///
/// 复用 [`ldown_api::service::link_unsupported`] 的稳定契约 message
...
fn manifest_item_to_preview_dto(
    ...
    ldown_api::types::PreviewItemDto {
        ...
        variants: item
            .variants
            .into_iter()
            .map(|v| ldown_api::types::PreviewVariantDto {
                id: v.id,
                label: v.label,
                size: v.size,
            })
...
mod tests {
    use super::*;
    use ldown_api::types::GroupItemRequest;
    ...
    async fn subscribe_task_events_delegates_to_the_shared_ws_hub() {
        use ldown_engine::events::{EngineEvent, EventSink};
        ...
        assert_eq!(ev.kind, ldown_api::service::TaskEventKind::Start);
    }
    ...
    async fn link_pairing_end_to_end_over_http() {
        ...
        fn info(name: &str) -> ldown_engine::link::SelfInfo {
            ldown_engine::link::SelfInfo {
                name: name.to_string(),
        ...
        let (tx_r, rx_r) = mpsc::channel::<ldown_engine::link::LinkEngineEvent>(16);
        ...
        let cfg = ldown_api::server::ApiServerConfig::from_config_map(
            &std::collections::HashMap::new(),
            "test",
        );
        let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind");
        let addr = listener.local_addr().expect("addr");
        let app = ldown_api::server::api_router(host, cfg);
        ...
        let (tx_i, _rx_i) = mpsc::channel::<ldown_engine::link::LinkEngineEvent>(16);
        ...
        tokio::spawn(async move {
            ...
            while let Some(ev) = rx.recv().await {
                if let ldown_engine::link::LinkEngineEvent::IncomingPairing {
                    session_id, ..
                } = ev
                {
    ...
    async fn link_management_plane_pair_and_devices_over_http() {
        ...
        async fn spawn_management(
            ...
            let cfg = ldown_api::server::ApiServerConfig::from_config_map(&map, "test");
            let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind");
            let addr = listener.local_addr().expect("addr");
            let app = ldown_api::server::api_router(host, cfg);
            tokio::spawn(async move {
        ...
        let begin: ldown_api::types::LinkPairBeginResponse = client
        ...
        // finish：SAS 核对后确认配对。
        let finish: ldown_api::types::LinkPairFinishResponse = client
        ...
        let devices: ldown_api::types::LinkDevicesResponse = client
        ...
        let del_body: ldown_api::types::LinkOkResponse = del.json().await.expect("delete json");
        assert!(del_body.ok);

        let devices_after: ldown_api::types::LinkDevicesResponse = client
            .get(format!("{base}/api/v1/link/devices"))
            .bearer_auth(token)
            .send()
            .await
    ...
    async fn link_pairing_rejected_by_responder_over_http() {
        ...
        assert!(
            matches!(err, ldown_engine::link::LinkError::RejectedByPeer),
