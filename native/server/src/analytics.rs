...
//! - 部署级硬关闭：环境变量 `LDOWNLOAD_ANALYTICS=off`（NAS/隐私敏感环境一刀切）。
//!
...
use ldown_engine::{log_error, log_info};

/// 编译期注入的 App-Key（CI：`LDOWNLOAD_ANALYTICS_APP_KEY`）。空 = 未配置。
const BAKED_APP_KEY: &str = match option_env!("LDOWNLOAD_ANALYTICS_APP_KEY") {
    Some(v) => v,
    None => "",
};

/// 统计上报端点。
const ENDPOINT: &str = "https://ops.dicad.cn/api/ldownload.v1.AnalyticsIngestService/TrackEvents";
...
fn app_key() -> String {
    match std::env::var("LDOWNLOAD_ANALYTICS_APP_KEY") {
        Ok(v) if !v.trim().is_empty() => v.trim().to_string(),
        _ => BAKED_APP_KEY.trim().to_string(),
    }
}

/// 部署级硬关闭：`LDOWNLOAD_ANALYTICS=off|0|false`。
fn disabled_by_env() -> bool {
    matches!(
        std::env::var("LDOWNLOAD_ANALYTICS").as_deref(),
...
pub async fn run(db: Db, server_version: &'static str) {
    if disabled_by_env() {
        log_info!("[analytics] disabled by LDOWNLOAD_ANALYTICS env");
        return;
    }
    let key = app_key();
    if key.is_empty() {
