...
//!   [`TaskEvent`](ldown_api::service::TaskEvent)（经
...
use ldown_api::service::{LiveSpeed, TaskEvent, TaskEventKind, task_event_for_transition};
use ldown_engine::events::{EngineEvent, EventSink};
use ldown_engine::log_info;
use ldown_engine::model::{BtFileEntry, HlsQualityOption, ResolveVariantOption, TaskInfo};
use ldown_engine::selection::{HostSelection, SelectionOutcome};
...
impl WsHub {
    ...
    /// [`ldown_api::service::ApiHost::subscribe_task_events`]。
...
mod tests {
    ...
    async fn engine_event_sink_maps_rss_items_changed_to_camel_case_json() {
        use ldown_engine::rss::model::{RssItemInfo, RssItemStatus};
