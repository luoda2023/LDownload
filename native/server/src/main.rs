//! LDownload headless 服务器 —— 复用 `ldown_engine` 下载引擎与
//! `ldown_api` HTTP 契约，外加 WebSocket 实时推送与 Web SPA 托管。
//!
//! 端到端：浏览器打开服务器地址 → token 登录 → 三栏任务界面管理下载 →
//! WS 实时进度 → 完成后经 `/api/v1/tasks/{id}/file` 流式取回。
//!
//! 运行：`cargo run -p ldown_server`（环境变量见 [`config`] 模块文档）。
...
use axum::Router;
use ldown_api::server::{ApiServerConfig, api_router};
use ldown_engine::db::Db;
use ldown_engine::download_manager::{self};
use ldown_engine::events::EventSink;
use ldown_engine::log_info;
...
/// 服务器版本。发布流水线在编译期经 `LDOWNLOAD_SERVER_VERSION` 注入 git tag
...
pub(crate) const SERVER_VERSION: &str = {
    let injected = match option_env!("LDOWNLOAD_SERVER_VERSION") {
        Some(v) => v,
        None => "",
    };
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 日志——日志须落到同一数据目录（`LDOWNLOAD_DATA_DIR`，Docker 下为挂载卷
    // `/data`），而非平台默认的 HOME 路径，才能持久化并让「关于」页正确显示。
    let data_dir =
        ldown_engine::data_dir::resolve_data_dir(server_cfg.data_dir_override.as_deref())?;
    ldown_engine::logger::init_with_dir(&data_dir);
    ...
    // LDOWNLOAD_LANG 是部署级默认语言：不写库，仅作设置页未保存过语言时的
    // 回退值（ServerApiHost::web_language 实时求值）——手动更改永远优先。
    if let Some(lang) = &server_cfg.language {
        log_info!("[server] default web language (LDOWNLOAD_LANG): {}", lang);
    }
    ...
    {
        ldown_engine::logger::set_max_total_bytes(mb * 1024 * 1024);
    }
    ...
    // 广播端口 = LDOWNLOAD_BIND 的端口；LDOWNLOAD_MDNS=off 时不主动广播（仍可手动配对）。
    let link_mgr = {
        let api_port = server_cfg
            ...
            .unwrap_or(17800);
        let self_name = std::env::var("LDOWNLOAD_LINK_NAME")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .unwrap_or_else(|| "LDownload Server".to_string());
            ...
            Ok(mgr) => {
                ...
                tokio::spawn(async move {
                    while let Some(ev) = link_rx.recv().await {
                        match ev {
                            ldown_engine::link::LinkEngineEvent::Discovered(_) => {}
                            ldown_engine::link::LinkEngineEvent::Paired(r) => {
                                log_info!(
                            ...
                            ldown_engine::link::LinkEngineEvent::Unpaired(_) => {
                                ws_hub.broadcast(&WsServerMsg::LinkDevicesChanged {});
                            }
                            ldown_engine::link::LinkEngineEvent::Error(m) => {
                                log_info!("[server] link error: {}", m);
                            }
                            ldown_engine::link::LinkEngineEvent::IncomingPairing {
                                session_id,
                                sas,
                                peer_name,
                                peer_platform,
                ...
                let mdns_on = std::env::var("LDOWNLOAD_MDNS")
                    .map(|v| {
                        !matches!(
    ...
    // LDOWNLOAD_ANALYTICS=off 或未配置 App-Key 时内部自行退出。
    ...
    // 路由：核心（ldown_api 复用）+ 扩展（本 crate）+ SPA 静态托管。
    ...
    let token_cell = ldown_api::auth::TokenCell::new(token.as_str());
    let api_cfg = ApiServerConfig {
        token: token_cell.clone(),
        management_enabled: true,
        ..api_cfg
    };
    let host: Arc<dyn ldown_api::service::ApiHost> = Arc::new(ServerApiHost::new(
        db_handle.clone(),
        cmd_tx.clone(),
        hub.clone(),
    ...
    // SPA：常态用二进制内嵌产物（单文件分发）；`LDOWNLOAD_WEBROOT` 显式指定时
    // 改从磁盘目录托管（自定义前端 / 本地调试）。
            ...
            if web_assets::is_embedded() {
                ...
                eprintln!(
                    "警告：本构建未嵌入 Web 界面（web/dist 缺失）。API 正常，浏览器打开会看到提示页；\n\
                     可先在 web/ 执行 `bun run build` 重新编译，或设置 LDOWNLOAD_WEBROOT 指向已构建目录。"
                );
            }
    ...
    eprintln!("LDownload Server listening on http://{}", server_cfg.bind);
