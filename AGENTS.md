# LDownload — AI 工作契约（核心）

多协议下载管理器（IDM 的免费替代）。官网 <https://dicad.cn>，版本号以 `pubspec.yaml` 为准。
**一套 Rust 下载引擎 `ldown_engine` + 多宿主 + 多客户端**：Flutter 桌面/移动 App、headless Web 服务器、CLI、WXT 浏览器扩展、Tampermonkey 用户脚本、JS 插件系统、内置 MCP/REST/aria2 API、React Web SPA。FFI 框架 [Rinf 8.10](https://rinf.cunarist.org)（bincode 信号）**仅** App（`hub` crate）用到。
...
**本文件所有路径以 `LDownload/` 为根**，命令按 cwd=`LDownload/` 书写。

- **cwd = `LDownload/`**（本文件自动加载）：路径与命令照抄即可。
- **cwd = 上级 `LDownloadProject/` 工作区**（那里的 `AGENTS.md` 自动加载，本文件按需 `read`）：所有路径前置 `LDownload/`；命令必须带目录限定（bash 工具 `cwd="LDownload"`、`cd LDownload && …`、`--manifest-path LDownload/…`），git 一律 `git -C LDownload …`。**工作区根既不是 git 仓库也不是工程根**，不带限定必然报错；bash 的 cwd 不跨调用保持。

两份文档分工，互不复制：
- 跨仓地图与跨仓契约（FluxCloud `/api/v1`、插件索引仓、主题仓、发布镜像 secret）→ 上级 `../AGENTS.md`。
- LDownload 内部架构 / 不变式 / 命令 → 本文件；**内部技术细节以本文件为准**（离代码最近）。
- 两边都写的红线（git 授权门槛、分支模型、禁用命令、i18n 基线）语义必须一致；LDownload 被单独 clone 时本文件自洽，不依赖上级文件存在。
...
cargo nextest run -p ldown_engine <filter>   # 引擎单测（协议/分段/DB）
cargo test -p ldown_api            # HTTP API（axum/aria2/MCP/OpenAPI 漂移守卫）
cargo test -p ldown_server         # headless server（WS/actor/扩展路由）
cargo test -p ldown_cli            # CLI（退出码/尺寸解析 doctest）
flutter test                          # Dart 测试
PG_TEST_URL=postgres://postgres:pw@localhost/postgres cargo test -p ldown_engine -- --ignored pg_smoke
# 插件相关（feature 门控）：
cargo test -p ldown_engine --features plugins,components --test plugin_ffmpeg   # 真实执行经 LDOWNLOAD_TEST_FFMPEG=<abs> 注入
cargo test -p ldown_engine --features plugins,components --test plugin_ytdlp    # LDOWNLOAD_TEST_YTDLP=<abs>
cargo check -p ldown_engine        # 不带 feature：验证 mobile 关插件时主链路零变化

# ── 各宿主/客户端运行 ──
cargo run -p ldown_server          # headless 服务器（env 见 .omp/knowledge/hosts-and-api.md）
cargo run -p ldown_cli -- ping     # CLI 探活（子命令同上文件）
cargo run -p ldown_cli -- add <url> --local   # B 模式：内嵌引擎独立下载
...
cd ldownload && npm run dev            # 扩展开发（Chrome）；dev:firefox / build / zip

# ── OpenAPI / 图标 / 发布 ──
cargo run -p ldown_api --example gen_openapi > website/public/openapi.json   # 改 API 后重生成
bun scripts/gen_icons.ts              # 改 assets/logo/ldownload_logo.svg 后全平台图标一键生成
...
**一个引擎，多个宿主，多个客户端。** 所有下载逻辑集中在 `ldown_engine`（`native/engine`，零 FFI/零 rinf），经三个 trait 与外界解耦：
...
- 两个生产宿主：`hub`（App，actor=`download_actor.rs`）、`server`（headless，actor=`actor.rs`）；`ldown_api` 只依赖 `&dyn ApiHost`，同一套 HTTP 面服务任意宿主。CLI 双模式：默认 HTTP 连宿主，`add --local` 内嵌引擎。
...
**crate 边界**
- `ldown_engine`：零 rinf/Dart/axum 依赖，只经 `EventSink`/`HostSelection` 与宿主解耦。协议/分段/DB/队列/组/插件全在这里。
- `ldown_api`：只依赖 `&dyn ApiHost`，定义 wire 契约 + 路径常量 + HTTP 服务器。零 rinf。
...
- **`ldown_server` 的 Web UI 是编译期内嵌的**：`native/server/build.rs` 把 `LDOWNLOAD_EMBED_WEBROOT`（缺省 `web/dist`）整棵目录递归全量 `include_bytes!` 进二进制（不按扩展名筛选，新增文件/新建子目录下次编译自动进包）。改了前端**必须先 `cd web && bun run build` 再重编服务器**才能看到；产物是单二进制，不再有同级 `web/` 目录，`LDOWNLOAD_WEBROOT` 降级为可选的磁盘覆盖。构建时目录缺失只 warning + 运行期 503 提示页，不会让编译失败。
...
| 任一 UI 文案 | 只补 **en + zh 基线对**：App `assets/i18n/{en,zh}.json` + `translations.dart` getter；`web/src/lib/locales/{en,zh}.json`；`website/src/lib/locales/{en,zh-CN}.json`；`ldownload/utils/locales/{en,zh-CN}.ts`。社区语言（`ja` 等）由 Weblate 维护，**不碰**（运行时键级回退英文） |
